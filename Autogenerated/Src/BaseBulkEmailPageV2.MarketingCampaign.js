		...
		function(resources, informationButtonResources, Enums, BusinessRuleModule, MarketingEnums, CampaignEnums,
			...
			return {
				...
				methods: {
					 ...
					 * @private
					 */
					_redirectToOtherEmailPage: function(config, callback) {
						this.callService(config, function(response) {
							if (!response.success) {
								this.showConfirmationDialog(this.get("Resources.Strings.OpenDesignerErrorMessage"),
									this.Terrasoft.emptyFn,	[this.Terrasoft.MessageBoxButtons.OK]);
								return;
							}
							var url = Terrasoft.workspaceBaseUrl + '/' + response.emailPageUrl;
							MarketingCommonUtilities.ShowConfirmationDialogWithRedirectButton(
								this.get("Resources.Strings.RedirectQuestionMessage"),
								function(returnCode) {
									if (returnCode === "redirectButton") {
										window.open(url, "_self");
									}
								},
								this.get("Resources.Strings.RedirectButtonCaption"),
								this
						);
						}, this);
					},
					 ...
					 * @private
					 */
					_checkIfDesignerTypeIsCorrect: function(callback) {
						var designerType = this.get("EmailDesignerType");
						if (!designerType || designerType.value === MarketingEnums.EmailDesignerType.WIZARD_EMAIL_DESIGNER) {
							callback.call(this);
							return;
						}
						var isBulkEmail = this.get("CategoryId") === MarketingEnums.BulkEmailCategory.MASS_MAILING ;
						var config = {
							serviceName: "MailingService",
							methodName: "GetEmailPageUrl",
							data: {
								data: {
									designerTypeId: designerType.value,
									bulkEmailId: this.getPrimaryColumnValue(),
									isBulkEmail: isBulkEmail
								}
							}
						};
						this._redirectToOtherEmailPage(config, callback);
					},
					...
					editTemplate: function() {
						...
						this._checkIfDesignerTypeIsCorrect(function() {
							var status = this.get("Status");
							if (status.value === MarketingEnums.BulkEmailStatus.ACTIVE) {
								var message = this.get("Resources.Strings.EditTemplateConfirmation");
								this._executeWithConfirmation(message,
