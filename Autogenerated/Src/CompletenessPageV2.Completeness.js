define("CompletenessPageV2", ["ServiceHelper", "CompletenessPageV2Resources", "css!CompletenessCSSV2"],
	function(ServiceHelper, resources) {
		return {
			...
			methods: {
				_get7xEntityStructure: function() {
					let entitySchemaName = this.get("EntitySchemaName");
					return Object.values(Terrasoft.configuration.ModuleStructure).find(x => !x.section8X && x.entitySchemaName === entitySchemaName);
				},
				
				_getIsCompletenessAllowed: function () {
					return this._get7xEntityStructure();
				},
				...
				onEntityInitialized: function() {
					if (!this._getIsCompletenessAllowed()) {
						this.showInformationDialog(resources.localizableStrings.No7xEditPagesExistForThisObject, function() {
				...
				subscribeSandboxEvents: function() {
					this.callParent(arguments);
					const tag = this.getDetailId("CompletenessParameters");
					this.sandbox.subscribe("GetMasterRecordInfo", this.onGetMasterRecordInfo, this,
						[tag]);
					this.sandbox.subscribe("GetEntityStructureFromPage", this._get7xEntityStructure, this,
						[tag]);
				},
