namespace Terrasoft.Configuration
{
	...
	public class CampaignAnalyticsProvider
	{
		...
		private Select GetCampaignTopParticipantSelect(IEnumerable<Guid> campaignIds) {
			var select = new Select(UserConnection)
				.Top(1)
				.Column("Id")
				.From("CampaignParticipant")
				.Where("CampaignId").In(Column.Parameters(campaignIds))
				.Union(new Select(UserConnection)
				.Column("Id")
				.From("CampaignParticipantOp")
				.Where("CampaignId").In(Column.Parameters(campaignIds))) as Select;
			select.SpecifyNoLockHints();
			return select;
		}
		...
		/// <summary>
		/// Returns True when specified campaigns have participants.
		/// </summary>
		/// <param name="campaignId">Id of campaign.</param>
		/// <returns>False for campaigns without audience.</returns>
		public virtual bool HasParticipants(IEnumerable<Guid> campaignIds) {
			var recordsCount = campaignIds.Count();
			var batchSize = 500;
			int processedCount = 0;
			var result = false;
			while (processedCount < recordsCount) {
				var participantsSelect = GetCampaignTopParticipantSelect(
					campaignIds.Skip(processedCount).Take(batchSize)
				);
				var topParticipant = participantsSelect.ExecuteScalar<Guid>();
				processedCount += batchSize;
				result = topParticipant.IsNotEmpty();
				if (result) {
					break;
				}
			}
