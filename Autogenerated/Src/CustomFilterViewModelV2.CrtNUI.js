	...
	function(Ext, Terrasoft, resources, ConfigurationEnums, CustomFilterView, quickFilterViewResources,
		...
		function initFiltersContainer() {
			...
			if (this.get("isExtendedFolderContainerExpanded")) {
				...
				let options = null;
				if (Terrasoft.Features.getIsDisabled("Disable7xCombinedModeCustomFilterClosePageIn8xHost")) {
					options = {
						initialization: true
					};
				}
				this.editExtendFilter(options);
			}
		}
		...
		function editExtendFilter(options = null) {
			this.goToExtendedMode(options);
		}
		...
		function goToExtendedMode(options= null) {
			var sandbox = this.getSandbox();
			this.set("isExtendedModeHidden", false);
			this.set("isFoldersHidden", true);
			this.set("ActiveFolder", null);
			sandbox.publish("CustomFilterExtendedMode", options, [sandbox.id]);
		}
