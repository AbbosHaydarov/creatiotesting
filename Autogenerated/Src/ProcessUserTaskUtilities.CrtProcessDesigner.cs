namespace Terrasoft.Configuration
{
	...
	using LogicalOperation = Terrasoft.Common.LogicalOperation;
	...
	public sealed class ProcessUserTaskUtilities : BaseProcessUserTaskUtilities
	{
		...
		#region Class: AllowedResult

		/// <summary>
		/// Represents information to serialization for model
		/// </summary>
		public class AllowedResult
		{
		...
		public static string GetAllowedActivityResults(UserConnection userConnection, Guid activityId) {
			...
			var allowedResults = new List<AllowedResult>();
			foreach (Entity entity in entityCollection) {
				allowedResults.Add(new AllowedResult {
					ResultId = entity.GetTypedColumnValue<Guid>("Id"),
					Caption = entity.GetTypedColumnValue<string>(columnName),
					CategoryId = entity.GetTypedColumnValue<Guid>("CategoryId")
				});
			}
			var serializeSettings = new JsonSerializerSettings {
				ContractResolver = new CamelCasePropertyNamesContractResolver()
			};
			return JsonConvert.SerializeObject(allowedResults, serializeSettings);
		}
		...
		/// <summary>
		/// Method is obsolete. Fetches top primary column values by filter.
		/// </summary>
		/// <param name="userConnection">User connection.</param>
		/// <param name="entitySchemaName">Name of the entity schema to read from.</param>
		/// <param name="serializedFilters">Serialized filters.</param>
		/// <param name="serializedSortingColumns">Serialized collection of the <see cref="SortingColumn"/> values.
		/// </param>
		/// <param name="topRecords">How many records to read.</param>
		/// <param name="skipRecords">How many records to skip.</param>
		/// <param name="excludeIds">List of the primary column values to exclude.</param>
		/// <returns>List of primary column values.</returns>
		[Obsolete("8.1.1|Method is deprecated, use IDataContractQueryExecutor instead.")]
		public static IList<Guid> FetchPrimaryColumnValues(UserConnection userConnection,
				string entitySchemaName, string serializedFilters, string serializedSortingColumns,
				int topRecords, int skipRecords, List<Guid> excludeIds) {
			var executorFactory = ClassFactory.Get<DataContractQueryExecutorFactory>();
			IDataContractQueryExecutor executor = executorFactory(userConnection);
			var sortingColumns = JsonConvert.DeserializeObject<List<SortingColumn>>(serializedSortingColumns);
			var dataContractQuery = new DataContractQuery {
				...
				ExcludedPrimaryColumnValues = excludeIds
			};
			return executor.ReadPrimaryColumnValues<Guid>(dataContractQuery);
		}
