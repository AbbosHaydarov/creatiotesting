 namespace Terrasoft.Mail.Sender
{
	...
	using Terrasoft.Configuration.EWS;
	...
	public class ExchangeClient : IEmailClient
	{
		...
		public ExchangeClient(UserConnection userConnection) {
			...
			_ignoreSslWarnings = (bool)SysSettings.GetValue(userConnection, "IgnoreExchangeSSLWarnings", true);
		}
		...
		private Exchange.EmailMessage CreateExchangeEmailMessage(EmailMessage bpmEmailMessage, bool ignoreRights) {
			...
			Info(string.Format("[ExchangeClient: {0} - {1}] Message \"{2}\" created", _userConnection.CurrentUser.Name,
				bpmEmailMessage.From.MaskedEmail(), bpmEmailMessage.Subject.Masked()));
		...
		protected virtual void SendMessage(Exchange.EmailMessage exchangeEmailMessage) {
			string emailAddress = exchangeEmailMessage.Sender.Address.ExtractEmailAddress();
			try {
				if (_userConnection.GetIsFeatureEnabled("SendFromPrimaryMailbox")) {
					if (_userConnection.GetIsFeatureEnabled("SendWithoutWaitingResponse")) {
						exchangeEmailMessage.SendAndSaveCopy();
					} else {
						exchangeEmailMessage.SendAndSaveCopy().Wait();
					}
				} else {
					...
					if (_userConnection.GetIsFeatureEnabled("SendWithoutWaitingResponse")) {
						...
						exchangeEmailMessage.SendAndSaveCopy(id).Wait();
					}
				}
			} catch (Exception e) {
				Info(string.Format("[ExchangeClient: {0} - {1}] Error on send message \"{2}\", {3}", _userConnection.CurrentUser.Name,
					emailAddress.MaskedEmail(), exchangeEmailMessage.Subject.Masked(), e.ToString()));
				throw;
			}
		}
		...
		public void Send(EmailMessage emailMessage, bool ignoreRights) {
			Info(string.Format("[ExchangeClient: {0} - {1}] Start sending message \"{2}\"", _userConnection.CurrentUser.Name,
				emailMessage.From.MaskedEmail(), emailMessage.Subject.Masked()));
