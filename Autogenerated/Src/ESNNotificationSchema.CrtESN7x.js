		...
		return {
			...
			methods: {
				...
				removeHtmlTags: function(value) {
					...
					value = value.replace(/<span([^>]+?)display:none;(.+?)>(.+?)<\/span>/gi, "");
					value = value.replace(/<span([^>]+?)border-radius:(\s?)50%(.+?)>(.+?)<\/span>/gi, "");
				...
				getSocialMessageText: function() {
					...
					if (this.getIsFeatureEnabled("DisableDecodeHtmlEntitiesForEsnNotifications")) {
						return Terrasoft.utils.string.decodeHtml(result);
					}
					return Terrasoft.decodeHtmlEntities(result);
				},
