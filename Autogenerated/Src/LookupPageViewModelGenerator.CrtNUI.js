	...
	const methods = {
		...
		loadSelected: function() {
			...
			select.getEntityCollection(function(response) {
				...
				const primaryColumnName = this.getCurrentSchema()?.primaryColumn?.name;
				const loadedIds = response?.collection?.getItems()
					.map((item) => item.get(primaryColumnName))
					.filter(Boolean);
				const selectedRecords = loadedIds || restoredIds;
				this.onLoadData(response, {mode: "top"});
				this.setSelectedRecords(selectedRecords);
			}, this);
		},
