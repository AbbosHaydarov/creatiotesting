namespace Terrasoft.Configuration.Omnichannel.Messaging
{
	...
	public class OmnichannelOutcomeMessagingService : OmnichannelMessagingService
	{
		...
		private OmnichannelMessagesRestrictionsHandler _messageHandler;
		/// <summary>
		/// Class which check messages <see cref="OmnichannelMessagesRestrictionsHandler"/> class.
		/// </summary>
		private OmnichannelMessagesRestrictionsHandler MessageHandler {
			get {
				_messageHandler = _messageHandler ?? new OmnichannelMessagesRestrictionsHandler(UserConnection);
		...
		public OmnichannelMessagingServiceResponse SendMessageWithAttachments() {
			...
			try {
				...
				if (MessageHandler.CheckLargeAttachmentsRestrictions(message)) {
					message.ChatId = ChatRepository.AddMessage(message, out bool isNewChat).ToString();
					result = MessageManager.Send(message, message.ChannelName);
				}
				stopWatch.Stop();
				metric.Duration = stopWatch.ElapsedMilliseconds;
				metric.Size = message.GetSize();
