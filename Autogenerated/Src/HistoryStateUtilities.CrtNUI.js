define("HistoryStateUtilities", ["ConfigurationEnums", "ConfigurationEnumsV2"], function(ConfigurationEnums) {
	...
	Ext.define("Terrasoft.configuration.mixins.HistoryStateUtilities", {
		...
		/**
		 * Returns the schema name from the history state.
		 * @return {String} Schema name.
		 */
		getSchemaNameFromHistoryState: function () {
			const historyState = this.sandbox.publish("GetHistoryState");
			const historyStateInfo = this.parseHistoryState(historyState.hash.historyState);
			return historyState.state.schemaName ?? historyStateInfo.schemas.pop();
		},
