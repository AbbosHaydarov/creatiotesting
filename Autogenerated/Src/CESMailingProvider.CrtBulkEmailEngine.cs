namespace Terrasoft.Configuration
{
	...
	public class CESMailingProvider : ISessionedMailingProvider, IConfigurableMailingProvider, ITestMessageProvider,
		...
		IMailingCesApiProvider, IExtendedMessageValidator
	{
		...
		private BpmonlineCloudEngine CloudEngine {
			get {
				...
				if (_serviceApi != null) {
					return _cloudEngine = ClassFactory.Get<BpmonlineCloudEngine>(
						new ConstructorArgument("userConnection", UserConnection),
						new ConstructorArgument("serviceApi", _serviceApi));
				}
				return _cloudEngine = ClassFactory.Get<BpmonlineCloudEngine>(
					new ConstructorArgument("userConnection", UserConnection));
			}
		...
		private MacrosHelperV2 MacrosHelper {
			get {
				if (_macrosHelper != null) {
		...
		private MailingResponse CreateErrorMailingResponse() {
			return new MailingResponse {
				...
				ValidationResults = new List<ValidationResultWrapper>()
			};
		}
        ...
        private BulkEmail FetchBulkEmailWithColumnsFromDB(Guid bulkEmailId, out bool fetchBulkEmailResult) {
            var bulkEmail = new BulkEmail(UserConnection);
            var entitySchemaManager = UserConnection.EntitySchemaManager;
            var entitySchema = entitySchemaManager.GetInstanceByName("BulkEmail");
            var columnsToFetch = entitySchema.Columns.Where(x => x.Name != "TemplateConfig");
            fetchBulkEmailResult = bulkEmail.FetchFromDB(entitySchema.PrimaryColumn, bulkEmailId, columnsToFetch);
			return bulkEmail;
        }
		
		private BulkEmail GetBulkEmailFromDB(Guid bulkEmailId) {
            var bulkEmail = FetchBulkEmailWithColumnsFromDB(bulkEmailId, out bool fetchBulkEmailResult);
		...
		private void RegisterSenderDomain(string senderEmail) {
			try {
				var macrosInfos = MacrosHelper.ExtractMacrosCollectionFromText(senderEmail);
				if (!macrosInfos.IsNullOrEmpty()) {
					return;
				}
		...
		private Guid SetBulkEmailBreakingStatus(BulkEmail bulkEmail) {
			...
			try {
				...
				bulkEmail.SetColumnValue("StatusId", stoppedStatusId);
				bulkEmail.SetColumnValue("StartDate", null);
				var isSaved = bulkEmail.Save();
				if (stoppedStatusId == MailingConsts.BulkEmailStatusBreakingId ||
					(stoppedStatusId == MailingConsts.BulkEmailStatusHardStoppedId && isSaved)) {
		...
		protected MailingStateExecutionResult ValidateBulkEmailInternal(BulkEmail bulkEmail) {
			MailingContext context = CreateMailingContext(bulkEmail);
			context.ChangeState(new ValidationMailingState());
			MailingStateExecutionResult executionResult = context.Execute();
			HandleMailingStepResult(bulkEmail, executionResult, false);
		...
		public MailingResponse BreakMailing(Guid bulkEmailId) {
			var bulkEmail = FetchBulkEmailWithColumnsFromDB(bulkEmailId, out var fetchResult);
		...
		public MailingResponse SendMessage(Guid bulkEmailId, bool isDelayedStart = false, string applicationUrl = "") {
			...
			var validationResult = ValidateBulkEmailInternal(bulkEmail);
            if (!validationResult.Success) {
				response.ValidationResults = validationResult.ValidationResults;
				return response;
			}
		...
		/// <returns>Response, which contains validation result</returns>
		public MailingValidationResponse ValidateMessageExtended(ValidationRequest validationRequest) {
			if (validationRequest.SendingType == SendingType.None) {
				var bulkEmail = FetchBulkEmailWithColumnsFromDB(validationRequest.MessageIds.FirstOrDefault(), out _);
				validationRequest.SendingType = bulkEmail.CategoryId == MailingConsts.TriggeredEmailBulkEmailCategoryId
					? SendingType.TriggerEmail
					: SendingType.BulkEmail;
			}
			return Validator.ValidateMessages(validationRequest.MessageIds, validationRequest.RecipientsCount,
				validationRequest.SendingType);
		}
