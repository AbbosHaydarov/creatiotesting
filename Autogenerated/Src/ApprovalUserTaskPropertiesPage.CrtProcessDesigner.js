		...
		methods: {
			...
			_saveSysModuleVisaParameters: function(element) {
				...
				if (this.get("IsSysApprovalStorageFeatureEnabled")) {
					...
					const sectionItem = this.Terrasoft.SectionManager.findByFn((item) => {
						if (entitySchema) {
							return item.getCode() === entitySchema?.name && item.sysModuleVisa;
						} else {
							return item.getId() === sectionId && item.sysModuleVisa;
						}
					});
			...
			_getSectionByEntitySchema: function(entitySchemaUId, callback, scope) {
				Terrasoft.SysModuleEntityManager.findItemsByEntitySchemaUId(entitySchemaUId, function(items) {
					var section;
					var sections = this.Ext.create("Terrasoft.Collection");
					items.each(function(item) { 
						var sysModuleEntityId = item.getId();
						var foundItems = Terrasoft.SectionManager.getItems().filterByFn(function(foundItem) {
							return foundItem.getSysModuleEntityId() === sysModuleEntityId;
						}, this);
						sections.collection.addAll(foundItems.getItems());
					}, this);
					section = sections.findByFn(function(item) {
						return item.sysModuleVisa;
					}, this) || sections.first();
			...
			initSection: function(callback, scope) {
				Terrasoft.chain(
					...
					function(next, section) {
						...
						} else if (entitySchemaUId) {
							const entitySchema = this.get("EntitySchemaList")
								.findByFn((entity) => entity.uId === entitySchemaUId) || {};
							this.set("SectionSelect", {
								value: entitySchema.id,
