define("TimezoneUtils", ["ServiceHelper", "BaseModule"], function(ServiceHelper) {
	...
	Ext.define("Terrasoft.configuration.TimezoneUtils", {
		...
		*/
		 ...
		 * @type {String}
		*/
		_adjustmentRulesCacheItemName: "AdjustmentRules",
		...
		getAdjustmentRuleResposeHandler: function(response) {
			...
			let rules = this.Terrasoft.ClientPageSessionCache.getItem(this._adjustmentRulesCacheItemName) || {};
			if (!this.Ext.isObject(rules[this._timezoneCode])) {
				rules[this._timezoneCode] = adjustmentRule;
				this.Terrasoft.ClientPageSessionCache.setItem(this._adjustmentRulesCacheItemName, rules);
			}
			this._timezoneCode = null;
		...
		getAdjustmentRule: function(timezoneCode, callback, scope) {
			const cachedRules = this.Terrasoft.ClientPageSessionCache.getItem(this._adjustmentRulesCacheItemName);
			if (this.Ext.isObject(cachedRules) && this.Ext.isObject(cachedRules[timezoneCode])) {
				const cachedTimezone = cachedRules[timezoneCode];
				this.Ext.callback(callback, scope, [cachedTimezone]);
				return cachedTimezone;
			}
			this._timezoneCode = timezoneCode;
			const serviceConfig = {
