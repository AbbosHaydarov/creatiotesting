namespace Terrasoft.Core.Process.Configuration
{
	...
	using Terrasoft.Configuration.CrtProcessDesigner;
	...
	public static class EmailTemplateUserTaskUtilities
	{
		...
		private static string GetContactEmailValue(UserConnection userConnection, object contactGuid) {
			var disableEmailFinder = userConnection.GetIsFeatureEnabled("DisableEmailFinder");
			if (!disableEmailFinder) {
				ClassFactory.TryGet("IContactEmailFinder", out IContactEmailFinder contactFinder);
				if (contactFinder != null) {
					return contactFinder.FindContactEmailValue(userConnection, contactGuid);
				}
			}
			var email = string.Empty;
		...
		private static string GetAccountEmailValue(UserConnection userConnection, object accountGuid) {
			var disableEmailFinder = userConnection.GetIsFeatureEnabled("DisableEmailFinder");
			if (!disableEmailFinder) {
				ClassFactory.TryGet("IAccountEmailFinder", out IAccountEmailFinder accountFinder);
				if (accountFinder != null) {
					return accountFinder.FindAccountEmailValue(userConnection, accountGuid);
				}
			}
			var email = string.Empty;
