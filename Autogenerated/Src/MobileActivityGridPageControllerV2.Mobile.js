...
Ext.define("Terrasoft.configuration.controller.ActivityGridPage", {
	...
	getStartWeekPeriod: function(date, days) {
		var period = {};
		var firstDayOfWeek = Ext.isEmpty(Terrasoft.CurrentUserInfo.firstDayOfWeek) ? 1 : Terrasoft.CurrentUserInfo.firstDayOfWeek;
		var dayOfWeek = date.getDay();
		var diff;
		if (dayOfWeek < firstDayOfWeek) {
			diff = 7 - (firstDayOfWeek - dayOfWeek);
		} else {
			diff = dayOfWeek - firstDayOfWeek;
		}
		period.startDate = Terrasoft.Date.add(date, Ext.Date.DAY, -diff);
