namespace Terrasoft.Configuration.Omnichannel.Messaging
{
	...
	public class ChatQueueRepository
	{
		...
		public List<OmniChatAction> GetQueueActions(Guid chatQueueId) {
			var esq = GetQueueActionsEsq(chatQueueId, out string oldProcessNameColumn);
			var processNameColumn = esq.AddColumn("BusinessProcess.Name").Name;
			...
			return collection.Select(entity => new OmniChatAction {
				ProcessName = entity.GetTypedColumnValue<string>(processNameColumn).IsNotNullOrEmpty() 
							? entity.GetTypedColumnValue<string>(processNameColumn)
							: entity.GetTypedColumnValue<string>(oldProcessNameColumn),
