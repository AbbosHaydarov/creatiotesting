define("SyncSettingsErrors", ["SyncSettingsErrorsResources", "SyncSettingsErrorItem", "SyncSettingsErrorsMixin",
	...
	return {
		attributes: {
			...
			/**
			 * Transient extra data for the tempate for specific mailbox.
			 * @Type {Terrasoft.BaseViewModelCollection}
			 */
			"MailboxesExtraErrorData": {
		...
		methods: {
			...
			_addGridLayoutItem: function(entity) {
				...
				const mailbox = {
					...
					impersonated: entity.get("UseImpersonation")
				};
			...
			onNewSyncError: function(scope, message) {
				...
				this.set("MailboxesExtraErrorData", [{mailboxId: mailboxId, extraData: receivedMessage.TransientExtraData}]);
			...
			createErrorItem: function(id, mailbox, textTpl, errorCode, action) {
				...
				var errorItem = this.Ext.create(errorItemClass, {
					...
					values: {
						...
						"Mailbox": mailbox,
						"TextTpl": textTpl,
						"ExtraTplData": this.get("MailboxesExtraErrorData")?.filter(x => x.mailboxId === id)[0]?.extraData,
						"ErrorCode": errorCode,
						"Action": action
					}
				});
			...
			addQueryColumns: function(settingsEsq) {
				...
				settingsEsq.addColumn("MailServer.UseImpersonation", "UseImpersonation");
