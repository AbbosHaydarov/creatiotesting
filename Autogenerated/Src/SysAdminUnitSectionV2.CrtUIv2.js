define("SysAdminUnitSectionV2", [
		...
		"performancecountermanager", "ProcessModuleUtilities", "RightUtilities", "GridUtilitiesV2", "css!AdministrationCSSV2", 
		"ActualizationUtilities"
	],
	function(ConfigurationConstants, ConfigurationEnums, PortalRoleFilterUtilities, resources, performanceManager, ProcessModuleUtilities,
			 RightUtilities) {
		return {
			...
			methods: {
				...
				init: function() {
					...
					this.addRoleBasedLicenseDistributionAction();
				},
				...
				/**
				 * Adds role-based license distribution action
				 * @protected
				 */
				addRoleBasedLicenseDistributionAction: function() {
					RightUtilities.checkCanExecuteOperation({
						...
						if(canExecuteOperation && featureEnabled) {
							var licenseRedistributionAction = this.getRedistributeLicencesForAllRolesAction();
							var actionMenuItems = this.get("CustomActionsButtonMenuItems");
				...
				/**
				 * Returns license redistribution buton.
				 * @protected
				 */
				getRedistributeLicencesForAllRolesAction: function() {
					var licenseRedistributionAction =  this.getActionsMenuItem({
						"Caption": {"bindTo": "Resources.Strings.RedistributeLicencesForAllRolesCaption"},
						"Click": {"bindTo": "redistributeLicencesForAllRoles"}
					});
					return licenseRedistributionAction;
				},
				
				/**
				 * Executes license redistribution for all roles
				 * @protected
				 */
				redistributeLicencesForAllRoles: function() {
				...
				licenseRedistributionHandler: function(result) {
                    ...
                    this.validateLicenseRedistribution(redistributeManuallyAssignedLicences);
				},
              
                validateLicenseRedistribution: function(redistributeManuallyAssignedLicences) {
					this.showBodyMask();
					var dataSend = {
						roleId: this.Terrasoft.GUID_EMPTY,
					...
					this.callService(config, function(response) {
						this.onvalidateLicenseRedistribution(response, redistributeManuallyAssignedLicences);
    					}, this);
				},
              
                onvalidateLicenseRedistribution: function(response, redistributeManuallyAssignedLicences) {
					this.hideBodyMask();
					if (!response
					...
					if (!success) {
						...
						if (!validationMessage) {
							this.runRedistributeLicensesProcess(redistributeManuallyAssignedLicences);
						}
						this.confirmRedistributionIfHasNoLicensedUsers(redistributeManuallyAssignedLicences, validationMessage);
					} else {
						this.runRedistributeLicensesProcess(redistributeManuallyAssignedLicences);
					}
				},
              
				confirmRedistributionIfHasNoLicensedUsers: function(redistributeManuallyAssignedLicences, validationMessage) {
				...
				runRedistributeLicensesProcess: function(redistributeManuallyAssignedLicences) {
                    ...
                    var processParameters = {
						...
						RedistributeLicencesForSpecificRole: false,
						RoleId: this.Terrasoft.GUID_EMPTY
					};
