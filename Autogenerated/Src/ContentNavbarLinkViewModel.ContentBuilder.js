define("ContentNavbarLinkViewModel", ["ContentNavbarLinkViewModelResources", "ckeditor-base", "InlineCodeTextEdit",
	...
	Ext.define("Terrasoft.ContentBuilder.ContentNavbarLinkViewModel", {
		...
		/**
		 * @inheritdoc Terrasoft.ContentElementBaseViewModel#elStyleNames
		 * @overridde
		 */
		elStyleNames: ["color", "font-family", "font-size", "line-height"],
		...
		constructor: function() {
			...
			this.on("change:Styles", this.actualizeElStyles, this);
			this.actualizeElStyles();
		},
		 ...
		 * @protected
		 */
		actualizeElStyles: function() {
			const elStyles = {};
			this.elStyleNames.forEach((x) => {
				elStyles[x] = this.$Styles[x];
			});;
			this.set("ElStyles", elStyles);
		}
	});
});
