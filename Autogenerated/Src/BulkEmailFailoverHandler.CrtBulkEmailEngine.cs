	...
	public class BulkEmailFailoverHandler : IFailoverHandler
	{
		...
		private Select CreateBulkEmailsSelect(UserConnection userConnection, int timeout) {
			...
			var select = new Select(userConnection)
					...
					.From("BulkEmail")
					.LeftOuterJoin("Campaign").On("Campaign", "Id")
					.IsEqual("BulkEmail", "CampaignId")
					.Where("BulkEmail", "StatusId").In(Column.Parameters(
						MarketingConsts.BulkEmailStatusStartsId, MarketingConsts.BulkEmailStatusWaitingBeforeSendId,
						MarketingConsts.BulkEmailStatusLaunchedId, MarketingConsts.BulkEmailStatusBreakingId,
						MailingConsts.BulkEmailStatusExpiredInProgressId))
					.And()
					.OpenBlock()
					.OpenBlock("Campaign", "CampaignSchemaUId").IsNull()
					.And()
					.OpenBlock("BulkEmail", "SendStartDate").IsLess(Column.Parameter(endCheckTime))
					.And("BulkEmail", "SendStartDate").IsGreater(Column.Parameter(startCheckTime))
					.CloseBlock()
					.CloseBlock()
					.Or()
					.OpenBlock("Campaign", "CampaignSchemaUId").Not().IsNull()
					.And("Campaign", "CampaignSchemaUId").IsNotEqual(Column.Parameter(Guid.Empty))
					.And("BulkEmail", "SendStartDate").IsGreater(Column.Parameter(startCheckTime))
					.And("BulkEmail", "SendStartDate").IsLess(Column.Parameter(endCheckTime))
					.And()
					.OpenBlock("BulkEmail", "SendDueDate").IsLess("BulkEmail", "SendStartDate")
					.Or("BulkEmail", "SendDueDate").IsNull()
					.CloseBlock()
					.CloseBlock()
					.CloseBlock()
				as Select;
