...
define("ContainerList", [], function() {
	...
	return Ext.define("Terrasoft.controls.ContainerList", {
		...
		initDomEvents: function() {
			this.callParent(arguments);
			this.debounceWindowScroll = this.debounceWindowScroll || Terrasoft.debounce(this.onWindowScroll.bind(this), 10);
			...
			this._subscribeContentResizing(wrapEl);
			this._subscribeContentScrolling(wrapEl);
		},

		_subscribeContentResizing: function(element) {
			if (!Terrasoft.isAngularHost || this._contentElResizeObserver || !element) {
				return;
			}
			const debounceContentElResize = Terrasoft.debounce(this._onResizeContentContainer, 100);
			this._contentElResizeObserver = new window.ResizeObserver(debounceContentElResize.bind(this));
			this._contentElResizeObserver.observe(element.dom);
		},

		_unsubscribeContentResizing: function() {
			this._contentElResizeObserver?.disconnect();
			this._contentElResizeObserver = null;
		},

		_onResizeContentContainer: function() {
			const wrapEl = this.getWrapEl();
			if (!wrapEl) {
				return;
			}
			const scrollableParent = this._findScrollableParent(wrapEl.dom);
			if (scrollableParent === this._scrollableParent || scrollableParent === window) {
				return;
			}
			this._scrollableParent.removeEventListener("scroll", this.debounceWindowScroll);
			this._scrollableParent.removeEventListener("mousewheel", this.debounceWindowScroll);
			// TODO RND-55895. Fix problem with Ext.EventManager.removeListeners
			scrollableParent.addEventListener("scroll", this.debounceWindowScroll);
			scrollableParent.addEventListener("mousewheel", this.debounceWindowScroll);
			this._scrollableParent = scrollableParent;
		},

		_subscribeContentScrolling: function(element) {
			this._unsubscribeContentScrolling();
			this._scrollableParent = this._findScrollableParent(element?.dom);
			this._scrollableParent.addEventListener("scroll", this.debounceWindowScroll);
			// for cases when scroll is in wrong place, just to force checkObservableRow for all children with mouse wheel
			this._scrollableParent.addEventListener("mousewheel", this.debounceWindowScroll);
			if (element) {
				element.dom?.addEventListener("scroll", this.debounceWindowScroll);
			}
		},

		_unsubscribeContentScrolling: function(element) {
			if (this._scrollableParent) {
				this._scrollableParent.removeEventListener("scroll", this.debounceWindowScroll);
				this._scrollableParent.removeEventListener("mousewheel", this.debounceWindowScroll);
			}
			if (element) {
				element.dom?.removeEventListener("scroll", this.debounceWindowScroll);
			}
		},
		 ...
		 * @private
		 */
		_findScrollableParent: function(el) {
		...
		onDestroy: function() {
			...
			this._unsubscribeContentResizing();
			this._unsubscribeContentScrolling(wrapEl);
			this.callParent(arguments);
		},
