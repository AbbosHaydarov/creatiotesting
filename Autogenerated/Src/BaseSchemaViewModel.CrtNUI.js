define("BaseSchemaViewModel", ["NetworkUtilities", "GoogleTagManagerUtilities", "BaseSchemaViewModelResources",
	...
	Ext.define("Terrasoft.configuration.BaseSchemaViewModel", {
		...
		getEditPages: function(operation) {
			...
			let editPages = this.get("EditPages");
			if (operation) {
		...
		getEditPageSchemaName: function(typeUId) {
			let editPage = this._getEditPage(typeUId);
			if (editPage) {
		...
		/**
		 * Gets the name of the entity edit mini page.
		 * @param {String} typeUId The value of the type column.
		 * @return {String} Returns the name of the entity edit mini page.
		 */
		getMiniPageSchemaName: function(typeUId) {
			const editPage = this._getEditPage(typeUId);
			if (editPage) {
				const miniPage = editPage.get("MiniPage");
				if (miniPage) {
					return miniPage.schemaName;
				}
			}
			return null;
		},
		...
		getLinkConfig: function(columnName) {
			...
			return {
				schemaName: cardSchema,
				cardEntitySchemaName: entitySchemaName,
				id: columnValue.value,
				operation: Terrasoft.ConfigurationEnums.CardOperation.EDIT,
		...
		openCardInChain: function(config) {
			...
			if (isAngularHostRouting && !config.excludeAngularRouting) {
		...
		_getCardModule: function(config) {
			...
			const entityName = config.cardEntitySchemaName || config.entitySchemaName || this.getSectionCode();
