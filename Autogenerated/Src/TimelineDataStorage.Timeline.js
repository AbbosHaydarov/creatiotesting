define("TimelineDataStorage", ["ConfigurationConstants", "BaseTimelineItemViewModel", "EsqTimelineDataProvider",
		"EsnTimelineDataProvider"],
	...
	Ext.define("Terrasoft.configuration.TimelineDataStorage", {
		...
		_prepareEmailItems: function(queryResult) {
			var result = Ext.create("Terrasoft.Collection");
			Terrasoft.each(queryResult.rows, function(row) {
				if (row.EntitySchemaName === "Activity" 
						&& row.EntityTypeColumnValue.value === ConfigurationConstants.Activity.Type.Email) {
					var item = result.findByPath("Id", row.Id) || row;
					...
					result.addIfNotExists(item.Id, item);
				} else {
					result.addIfNotExists(row.Id, row);
				}
			}, this);
			return result;
		},
		...
		_loadEntityItems: function(queryResult) {
			var rows = this._prepareEmailItems(queryResult);
