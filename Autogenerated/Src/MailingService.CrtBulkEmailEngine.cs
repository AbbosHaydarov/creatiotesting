namespace Terrasoft.Configuration
{
	...
	public class MailingService : BaseService, IReadOnlySessionState
	{
		...
		/// <summary>
		/// Gets the email page url URL.
		/// </summary>
		[OperationContract]
		[WebInvoke(Method = "POST", UriTemplate = "GetEmailPageUrl",
			BodyStyle = WebMessageBodyStyle.WrappedRequest, RequestFormat = WebMessageFormat.Json,
			ResponseFormat = WebMessageFormat.Json)]
		public GetEmailPageUrlResponse GetEmailPageUrl(GetEmailPageUrlRequest data) {
			IEnumerable<IEmailPageUrlProvider> designerUrlProviders = ClassFactory
				.GetAll<IEmailPageUrlProvider>(new ConstructorArgument("userConnection", UserConnection))
				.Where(provider => provider.DesignerTypeId == data.DesignerTypeId)
				.ToList();
			try {
				if (!designerUrlProviders.Any() || designerUrlProviders.Count() > 1) {
					string errorMessage = designerUrlProviders.Count() > 1 ? "Multiple url providers found"
						: "Url provider not found";
					return new GetEmailPageUrlResponse {
						Success = false,
				...
				IEmailPageUrlProvider emailProvider = designerUrlProviders.Single();
				string designerUrl = emailProvider.GetEmailPageUrl(data);
				return new GetEmailPageUrlResponse {
					Success = true,
					EmailPageUrl = designerUrl
				};
			} catch (Exception e) {
				return new GetEmailPageUrlResponse {
					Success = false,
		...
		[WebInvoke(Method = "POST", UriTemplate = "ValidateMessage", BodyStyle = WebMessageBodyStyle.Wrapped,
			RequestFormat = WebMessageFormat.Json, ResponseFormat = WebMessageFormat.Json)]
		public MailingValidationResponse ValidateMessage(Guid messageId) {
			return (_mailingProvider as IExtendedMessageValidator).ValidateMessageExtended(new ValidationRequest {
		...
		[WebInvoke(Method = "POST", UriTemplate = "ValidateMessages", BodyStyle = WebMessageBodyStyle.Wrapped,
			RequestFormat = WebMessageFormat.Json, ResponseFormat = WebMessageFormat.Json)]
		public MailingValidationResponse ValidateMessages(Guid[] messageIds) {
			return (_mailingProvider as IExtendedMessageValidator).ValidateMessageExtended(new ValidationRequest {
		...
		[WebInvoke(Method = "POST", UriTemplate = "ValidateTestMessage", BodyStyle = WebMessageBodyStyle.Wrapped,
			RequestFormat = WebMessageFormat.Json, ResponseFormat = WebMessageFormat.Json)]
		public MailingValidationResponse ValidateTestMessage(Guid messageId, int recipientsCount) {
			return (_mailingProvider as IExtendedMessageValidator).ValidateMessageExtended(new ValidationRequest {
				MessageIds = new[] { messageId },
