namespace Terrasoft.Core.Process
{
	...
	public class SocialLeadGeneration_ConsistencyCheckMethodsWrapper : ProcessModel
	{
		...
		private bool ScriptTask3Execute(ProcessExecutingContext context) {
			try {
				var accountRestProvider = new AccountSocialLeadGenRestProvider(UserConnection);
				var integrationResponse = accountRestProvider.GetIntegrationList();
				Set("IntegrationExist", integrationResponse.Integrations.Any());
				Set("RequestSuccess", true);
			} catch (Exception exception) {
				Set("RequestSuccess", false);
				Set("ErrorMessage", exception.Message);
