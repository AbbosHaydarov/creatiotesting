define("CtiLinkColumnUtility", [], function() {
		...
		Ext.define("Terrasoft.configuration.CtiLinkColumnUtility", {
			...
			isTelephonyEnabled: function() {
				...
				if (!connectionParams || !ctiSettings.canUseCti || connectionParams.disableCallCentre) {
					return false; 
				};
				var storage = Terrasoft.configuration.Storage;
				var key = ctiSettings.licOperations?.join("-");
				var licStore = storage.UserOperationLicense || [];
				return !key || licStore[key];
			},
