define("FunnelBaseDataProvider", ["ext-base", "terrasoft", "OpportunityStage", "ChartModuleHelper",
	...
	function(Ext, Terrasoft, OpportunityStage, ChartModuleHelper, ConfigurationConstants, resources) {
		...
		Ext.define("Terrasoft.configuration.FunnelBaseDataProvider", {
			 ...
			 */
			getFunnelFixedFilters: function(stageColumnFilterValue) {
				...
				if (stageColumnFilterValue) {
					var stageColumnPath = this.getDetailColumnPath("Stage");
					esqFiltersGroup.add('drillDownFilters', Terrasoft.createColumnFilterWithParameter(
						Terrasoft.ComparisonType.EQUAL, stageColumnPath, stageColumnFilterValue));
				}
				return esqFiltersGroup;
			},

			/**
			 * Removes filter by path and value from filters collection.
			 * @param {Terrasoft.FilterGroup} filters Filter collection.
             * @param {string} columnPath Path to filter column.
             * @param {string} value Value of filter column.
			 */
          removePathFilter: function(filters, columnPath, value) {
				filters.findByFn(function(item) {
					var keyValue = item.leftExpression
					&& item.leftExpression.columnPath
					&& item.leftExpression.columnPath === columnPath
					&& item.rightExpression
					&& item.rightExpression.parameterValue
					&& item.rightExpression.parameterValue === value
						? item.key
						: undefined;
					if (keyValue) {
						filters.removeByKey(keyValue);
					}
				}, this);
			},
			...
			addFilters: function(entitySchemaQuery) {
				var stageColumnFilterValue;
				if (!this.isDrilledDown() ||
					(this.getDisplayMode() === Terrasoft.DashboardEnums.ChartDisplayMode.GRID)) {
						this.callParent(arguments);
						var drillDownFilters = this.getDrillDownFilters();
						if (drillDownFilters) {
							var stageColumnPath = this.getDetailColumnPath("Stage");
							stageColumnFilterValue = drillDownFilters[stageColumnPath];
							if (stageColumnFilterValue) {
								this.removePathFilter(entitySchemaQuery.filters, stageColumnPath, stageColumnFilterValue.value);
							}
						}

				}
				entitySchemaQuery.filters.removeByKey("xAxisColumnIsNotNullFilter");
				entitySchemaQuery.filters.addItem(this.getFunnelFixedFilters(stageColumnFilterValue));
			},
