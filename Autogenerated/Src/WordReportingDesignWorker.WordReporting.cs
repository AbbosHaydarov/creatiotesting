namespace Terrasoft.Configuration.Reporting.Word.Worker
{
	...
	public class WordReportingDesignWorker : IWordReportDesignWorker
	{
		...
		private DataObjectSettingsDto GetDataSettingsDto(Guid reportId) {
			...
			var newSysEntitySchemaUIdColumnName = esq.AddColumn("SysEntitySchema.Id").Name;
			var sysSchemaTitleColumnName =
				esq.AddColumn("SysEntitySchema.Title").Name;
			...
			var caption = entity.GetTypedColumnValue<string>(sysSchemaCaptionColumnName);
			if(string.IsNullOrEmpty(caption))
			{
				caption = entity.GetTypedColumnValue<string>(sysSchemaTitleColumnName);
			}
			var entitySchemaUId = entity.GetTypedColumnValue<Guid>(sysEntitySchemaUIdColumnName);
			if(entitySchemaUId.Equals(Guid.Empty))
			{
				entitySchemaUId = entity.GetTypedColumnValue<Guid>(newSysEntitySchemaUIdColumnName);
			}
		...
		public IEnumerable<WordReportMainInfoDto> GetReportsCollection() {
			...
			var sysEntitySchemaIdColumnName = esq.AddColumn("SysEntitySchema.Id").Name;
			var sysEntitySchemaTitleColumnName = esq.AddColumn("SysEntitySchema.Title").Name;
			esq.Filters.Add(ReportTypeFilter(esq));
			return esq.GetEntityCollection(_userConnection).Select(x => {
				var dto = new WordReportMainInfoDto
				{
				...
				if(dto.SysModuleId.Equals(Guid.Empty))
				{
					dto.SysModuleId = x.GetTypedColumnValue<Guid>(sysEntitySchemaIdColumnName);
				}
				if(string.IsNullOrEmpty(dto.SysModuleCaption))
				{
					dto.SysModuleCaption = x.GetTypedColumnValue<string>(sysEntitySchemaTitleColumnName);
				}
				return dto;
			});
		}
