namespace Terrasoft.Configuration
{
	...
	public class ReportEngine : IReportEngine
	{
		...
		private EntitySchemaQuery GetReportsEntitySchemaQuery() {
			...
			esq.AddColumn("SysEntitySchema.Name").Name = "EntitySchemaName";
			esq.AddColumn("SysModule.SysModuleEntity.[SysSchema:UId:SysEntitySchemaUId].Name")
				.Name = "SysModuleEntitySchemaName";
			esq.AddColumn("ConvertInPDF");
			return esq;
		}
		
		private string GetReportEntitySchemaName(Entity entity) {
			var entitySchemaName = entity.GetTypedColumnValue<string>("EntitySchemaName");
			if (entitySchemaName.IsNullOrEmpty()) {
				entitySchemaName = entity.GetTypedColumnValue<string>("SysModuleEntitySchemaName");
			}
		...
		private ReportTemplateInfo GetReportInfoFromDb(Guid id) {
			...
			if (entity != null) {
				var reportInfo = new ReportTemplateInfo {
					...
					EntitySchemaName = GetReportEntitySchemaName(entity),
