 namespace Terrasoft.Sync.Exchange
{
	...
	public class ExchangeAppointmentSyncProvider : ExchangeActivitySyncProvider
	{
		...
		private Exchange.Appointment GetAppointmentByExtraProperties(SysSyncMetaData activityMetadata) {
			var localId = activityMetadata.LocalId;
			var filters = new Exchange.SearchFilter.SearchFilterCollection(Exchange.LogicalOperator.Or);
			var localIdPropSetFilter = new Exchange.SearchFilter.IsEqualTo(ExchangeUtilityImpl.LocalIdProperty, localId.ToString());
			var remoteIdPropSetFilter = new Exchange.SearchFilter.IsEqualTo(ExchangeUtilityImpl.RemoteIdProperty, activityMetadata.RemoteId);
			filters.Add(localIdPropSetFilter);
			filters.Add(remoteIdPropSetFilter);
		...
		private void UnlockSyncEntity(SyncContext context, IRemoteItem remoteItem, LocalItem localItem) {
			var helper = ClassFactory.Get<EntitySynchronizerHelper>();
			foreach (var syncEntity in localItem.Entities) {
				foreach (var syncEntity1 in syncEntity.Value) {
					helper.UnlockEntity(syncEntity1.EntityId, context.UserConnection.CurrentUser.ContactId,
						"Exchange integration", context.UserConnection);
				}
			}
		}

		private void SyncDeletedAppointments(SyncContext context, IRemoteItem remoteItem, LocalItem localItem) {
		...
		public override IRemoteItem LoadSyncItem(SyncItemSchema schema, ItemMetadata itemMetadata) {
			...
			if (appointment == null && _userConnection.GetIsFeatureEnabled("SyncDeletedAppointments")) {
				appointment = GetAppointmentByExtraProperties(syncItemMetadata);
			}
			if (appointment != null) {
				...
				bool isDeletedAndLocalCopy = syncItemMetadata.RemoteState == (int)SyncState.Deleted;
				remoteItem.Action = isDeletedAndLocalCopy ? SyncAction.None : SyncAction.Delete;
			}
			return remoteItem;
		}
		...
		/// </summary>
		public override void OnLocalItemAppliedInRemoteStore(SyncContext context, IRemoteItem remoteItem, LocalItem localItem) {
			UnlockSyncEntity(context, remoteItem, localItem);
			SyncDeletedAppointments(context, remoteItem, localItem);
		}
