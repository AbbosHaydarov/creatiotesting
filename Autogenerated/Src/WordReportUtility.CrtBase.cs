namespace Terrasoft.Configuration
{
	...
	public class WordReportUtility
	{
		...
		private string GetSchemaNameByTemplateId(Guid templateId) {
			...
			var entityNameColumnName = reportESQ.AddColumn("SysEntitySchema.Name").Name;
			var moduleEntityNameColumnName = reportESQ.AddColumn("SysModule.SysModuleEntity.SysEntitySchemaUId").Name;
			reportESQ.Filters.Clear();
			reportESQ.Filters.Add(
			reportESQ.CreateFilterWithParameters(FilterComparisonType.Equal, "Id", templateId));
			...
			if(reportCollection.Count > 0) {
				...
				var schemaColumnName = reportEntity.Schema.Columns.GetByName(entityNameColumnName);
				if (schemaColumnName.IsNullOrEmpty()) {
					schemaColumnName = reportEntity.Schema.Columns.GetByName(moduleEntityNameColumnName);
				}
