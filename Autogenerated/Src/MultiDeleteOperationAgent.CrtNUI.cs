namespace Terrasoft.Configuration
{
	...
	public class MultiDeleteOperationAgent : IOperationAgent, IRecordProcessedHandler
	{
		...
		private Insert GetQueueInsertQueryForVirtualSchema(Guid recordId) {
			var insertForVirtualSchema = new Insert(_userConnection).Into(MULTI_DELETE_QUEUE)
				.Set("EntitySchemaUId", Column.Parameter(EntitySchemaUId))
				.Set("RecordId", Column.Parameter(recordId))
				.Set("SysAdminUnitId", Column.Parameter(_userConnection.CurrentUser.Id))
				.Set("OperationKey", Column.Parameter(OperationKey))
				.Set("State", Column.Parameter(OperationState.Wait))
				.Set("IsRoot", Column.Parameter(IsRoot));
			return insertForVirtualSchema;
		}
		...
		protected virtual int InsertMultiDeleteQueue(IEnumerable<Guid> recordIds) {
			var entitySchema = this._userConnection.EntitySchemaManager.GetInstanceByName(this.EntitySchemaName);
			if (entitySchema.IsVirtual) {
				foreach (Guid recordId in recordIds) {
					var insertForVirtualSchema = GetQueueInsertQueryForVirtualSchema(recordId);
					insertForVirtualSchema.Execute();
				}
