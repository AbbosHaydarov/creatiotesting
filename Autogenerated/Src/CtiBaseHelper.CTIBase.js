define("CtiBaseHelper", ["terrasoft", "CtiConstants", "CtiBaseHelperResources"], function(Terrasoft, CtiConstants,
	...
	Ext.define("Terrasoft.configuration.CtiBaseHelper", {
		...
		queryCtiLicenses: function(ctiSettings, callback) {
			...
			const disableCallCentre = ctiSettings.connectionParams
				? ctiSettings.connectionParams.disableCallCentre
				: true;
			if (disableCallCentre) {
				ctiSettings.canUseCti = false;
				callback(ctiSettings);
				return;
			}
			...
			this.GetUserHasOperationLicense({
				...
				if (!canUseCti) {
					this.log(Ext.String.format(CtiConstants.LocalizableStrings.LicenseNotFoundMessage,
						licOperations), Terrasoft.LogMessageType.ERROR);
				}
				callback(ctiSettings);
			}, this);
		},
