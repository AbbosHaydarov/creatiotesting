		...
		return {
			...
			methods: {
				...
				_getBulkEmailEntity: function(bulkEmailId, callback) {
					...
					esq.addColumn("EmailDesignerType");
				...
				/**
				 * Prepare update query for set the email designer type.
				 * @protected
				 * @returns {Terrasoft.UpdateQuery} update.
				 */
				getUpdateEmailDesignerTypeQuery: function() {
					var update = Ext.create("Terrasoft.UpdateQuery", {
						rootSchemaName: "BulkEmail"
					});
					var recordId = this.get("RecordId");
					update.enablePrimaryColumnFilter(recordId);
					update.setParameterValue("EmailDesignerType", MarketingEnums.EmailDesignerType.WIZARD_EMAIL_DESIGNER,
				...
				onSaveAction: function(config) {
					this._getBulkEmailEntity(this.$BulkEmailEntity.$Id, function(bulkEmail) {
						if(!bulkEmail.$EmailDesignerType.value || bulkEmail.$EmailDesignerType.value != MarketingEnums.EmailDesignerType.WIZARD_EMAIL_DESIGNER) {
							var update = this.getUpdateEmailDesignerTypeQuery();
