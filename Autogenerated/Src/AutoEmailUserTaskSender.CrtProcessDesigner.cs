namespace Terrasoft.Core.Process.Configuration
{
	...
	public class AutoEmailUserTaskSender : IEmailUserTaskSender
	{
		...
		private void TrySendEmailWithRetry(EmailTemplateUserTask userTask, Action sendAction, Action onError = null) {
			try {
				sendAction();
			} catch (Exception e) {
				_log.Error(e);
				if (!userTask.IgnoreErrors) {
					if (e is WebException wex) {
						_log.Info("Attempt to retry send");
						Retry(sendAction, onError);
					} else {
						_log.Info("Skip retry send");
						onError?.Invoke();
						throw;
					}
				}
		...
		private void Retry(Action retriedAction, Action onError) {
			try {
				retriedAction();
				_log.Info("Retry is successful");
			}
			catch (Exception ex) {
				_log.Error("Retry error", ex);
				onError?.Invoke();
				throw;
			}
		}
		...
		private void SendEmailUsingActivity(EmailClientFactory emailClientFactory, EmailTemplateUserTask userTask,
			...
			if (FetchExistingActivity(userTask, out var existingActivity)) {
				TrySendEmailWithRetry(userTask, () => emailSender.Send(existingActivity, null, ignoreRights));
				...
				TrySendEmailWithRetry(userTask, () => emailSender.Send(createdActivity, message, ignoreRights),
		...
		private void SendEmail(EmailClientFactory emailClientFactory, EmailTemplateUserTask userTask,
			...
			TrySendEmailWithRetry(userTask, () => emailSender.Send(message, ignoreRights));
		}
