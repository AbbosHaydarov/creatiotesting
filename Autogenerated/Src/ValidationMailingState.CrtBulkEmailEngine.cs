namespace Terrasoft.Configuration
{
	...
	public class ValidationMailingState : MailingState
	{
		...
		protected override MailingStateExecutionResult HandleStepInternal() {
			...
			var validationResults = failedResults.Where(x => !string.IsNullOrEmpty(x.ValidationCode))
				.Select(x => new ValidationResultWrapper { 
					ValidationCode = x.ValidationCode, 
					ValidationMessage = GetLczStringValue(x.LczStringCode) 
				});
			result.ValidationResults = validationResults ?? new List<ValidationResultWrapper>();
