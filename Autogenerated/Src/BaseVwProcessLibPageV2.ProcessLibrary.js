...
define("BaseVwProcessLibPageV2", ["BaseVwProcessLibPageV2Resources", "GeneralDetails", "InformationButtonResources",
	...
	return {
		...
		attributes: {
			...
			/**
			 * Tracing automatic turn-off date.
			 */
			TracingTurnOffDate: {
				dataValueType: Terrasoft.DataValueType.DATE,
				caption: { bindTo: "Resources.Strings.TracingTurnOffDateCaption" },
				value: null
			},
			/**
		...
		methods: {
			 ...
			 * @private
			 */
			_initTracingTimeout: function(callback, scope) {
				const settingCode = "ProcessParameterTracingDisableTimeoutDays";
				Terrasoft.SysSettings.querySysSettingsItem(settingCode, function(value) {
					const daysToDisable = value ?? 5;
					this.set("TracingTimeoutDays", daysToDisable);
			...
			init: function(callback, scope) {
				this.callParent([
					function() {
						...
						this._initTracingTimeout(callback, scope)
					}, this
				]);
			...
			/**
			 * Determines whether the tracing auto turn-off is available or not.
			 * @protected
			 */
			isTracingTurnOffAvailable: function() {
				return this.get("TracingTimeoutDays") !== 0 && this.get("IsTracing");
			},
			 ...
			 * @private
			 */
			_initTracingTurnOffDate: function(modifiedOn) {
				let date = (modifiedOn)
					? Terrasoft.parseDate(modifiedOn, 'MS')
					: new Date();
				const daysToDisable = this.get("TracingTimeoutDays");
				if (daysToDisable === 0) {
					return;
				}
				date.setDate(date.getDate() + daysToDisable);
				const today = new Date();
				this.set("TracingTurnOffDate", date < today ? today : date);
			},
			...
			initTracingAttribute: function(callback, scope) {
				Terrasoft.SysUserPropertyHelper.getProperty({
					...
					if (response.success) {
						const isTracing = (/true/i).test(response.value);
						this.set("IsTracing", isTracing);
						if (isTracing) {
							this._initTracingTurnOffDate(response.modifiedOn);
						}
					} else {
