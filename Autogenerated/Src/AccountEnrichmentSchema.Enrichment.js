	...
	return {
		...
		methods: {
			...
			executeEnrichmentDataService: function(callback, scope) {
				this.log("executeEnrichmentDataService: start");
				...
				this.callService({
					...
					this.log("executeEnrichmentDataService: response");
			...
			onExecuteEnrichmentDataService: function(response, callback, scope) {
				...
				if (Terrasoft.isEmptyObject(response)) {
					this.log("onExecuteEnrichmentDataService: response empty");
					return;
				}
				const searchAccountInfoResult = response.result;
				if (searchAccountInfoResult.isSuccessSearch) {
					this.log("onExecuteEnrichmentDataService: isSuccessSearch");
					var callbackFunc = function() {
						this.log("onExecuteEnrichmentDataService: callbackFunc");
						if (Ext.isFunction(callback)) {
							callback.apply(scope || this, arguments);
						}
						if (!this.getIsNotEmptyCollection()) {
							this.log("onExecuteEnrichmentDataService: NoNewData");
