define("PrintReportUtilities", ["LocalizationUtilities", "SysModuleReport", "SysModuleAnalyticsReport",
	...
	function(LocalizationUtilities, SysModuleReport, SysModuleAnalyticsReport,
		...
		Ext.define("Terrasoft.configuration.mixins.PrintReportUtilities", {
			...
			_addEntitySchemaNameFilter: function(esq) {
				const entitySchemaName = this.getEntitySchemaName();
				const filter = Terrasoft.createFilterGroup();
				filter.logicalOperation = Terrasoft.LogicalOperatorType.OR;
				const sysEntitySchemaFilter = Terrasoft.createColumnFilterWithParameter(
					"SysEntitySchema.Name", entitySchemaName);
				const sysModuleEntityFilter = Terrasoft.createColumnFilterWithParameter(
					"SysModule.SysModuleEntity.[SysSchema:UId:SysEntitySchemaUId].Name", entitySchemaName);
				filter.add("sysEntitySchemaFilter", sysEntitySchemaFilter);
				filter.add("sysModuleEntityFilter", sysModuleEntityFilter);
				esq.filters.add("entitySchemaFilter", filter);
			},

			_addTypeFilter: function(esq) {
				...
				const filter = Terrasoft.createColumnInFilterWithParameters("Type.Name", supportedTypes);
				esq.filters.add("typeFilter", filter);
			},

			_prepareModuleReportsFilters: function(esq) {
				this._addEntitySchemaNameFilter(esq);
				this._addTypeFilter(esq);
			},
			...
			getModulePrintFormsESQ: function() {
				...
				this._prepareModuleReportsFilters(esq);
				return esq;
			},
