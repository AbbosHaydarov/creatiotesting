	...
	return {
		...
		methods: {
			...
			onEnableCaseClick: function() {
				...
				let dcmSettingsItem;
				if (moduleEntityId) {
					dcmSettingsItem = Terrasoft.SysDcmSettingsManager.findBySysModuleEntityId(moduleEntityId);
				} else {
					const targetSchemaUId = this.get("EntitySchemaUId");
					dcmSettingsItem = Terrasoft.SysDcmSettingsManager.findBySysSchemaUId(targetSchemaUId);
				}
