define("AlignableContainer", ["css!AlignableContainer"], function() {
	...
	Ext.define("Terrasoft.controls.AlignableContainer", {
		 ...
		 */
		defaultRenderTpl: [
			"<div id=\"{id}-overlay\" class=\"{overlayClassName}\" />",
			"</div>",
		...
		getTplData: function() {
			...
			if (!classes.overlayClassName) {
				classes.overlayClassName = [];
			}
			var overlayClasses = classes.overlayClassName;
			overlayClasses.push("alignable-container-overlay");
			if (this.showOverlay) {
				overlayClasses.push("show-overlay");
			}
		...
		setOverlayVisibility: function(visible) {
			...
			if (!this.overlayEl) {
				this.overlayEl = Ext.get(this.id + "-overlay");
			}
			var overlayEl = this.overlayEl;
			if (overlayEl) {
				if (visible) {
					overlayEl.addCls("show-overlay");
				} else {
					overlayEl.removeCls("show-overlay");
				}
			}
