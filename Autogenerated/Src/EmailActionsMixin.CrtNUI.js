define("EmailActionsMixin", ["ExchangeNUIConstants", "EmailConstants"],
	function(ExchangeNUIConstants, EmailConstants) {
		/**
		...
		Ext.define("Terrasoft.configuration.mixins.EmailActionsMixin", {
			...
			getColumnDisplayValue: function(item) {
				...
				return this.Ext.String.format(
					EmailConstants.emailFormatString,
					ownerName,
			...
			/**
			 * Sets 'X-In-Reply-To-Activity' header in HeaderProperties.
			 * @protected
			 * @param {String} parentActivityId Original email activity id.
			 */
			setInReplyToHeader: function(parentActivityId) {
				if (Ext.isEmpty(parentActivityId)) {
					return;
				}
				var inReplyToHeader = "X-In-Reply-To-Activity=" + parentActivityId;
				var headerProperties = this.get("HeaderProperties");
				headerProperties = headerProperties
						? headerProperties + "\n" + inReplyToHeader
						: inReplyToHeader;
				this.set("HeaderProperties", headerProperties);
			},
