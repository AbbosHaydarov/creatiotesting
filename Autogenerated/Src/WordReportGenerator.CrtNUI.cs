namespace Terrasoft.Configuration.WordReportGenerator
{
	...
	public class WordReportGenerator : IReportGenerator
	{
		...
		private ReportTemplateSettings GetReportTemplateSettings(UserConnection userConnection, Guid templateId) {
			...
			var entityNameColumnName = esq.AddColumn("SysEntitySchema.Name").Name;
			var moduleEntityNameColumnName = esq.AddColumn("SysModule.SysModuleEntity.[SysSchema:UId:SysEntitySchemaUId].Name").Name;
			...
			var entitySchemaName = entity.GetTypedColumnValue<string>(entityNameColumnName);
			if (entitySchemaName.IsNullOrEmpty()) {
				entitySchemaName = entity.GetTypedColumnValue<string>(moduleEntityNameColumnName);
			}
