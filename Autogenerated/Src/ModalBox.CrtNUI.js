define("ModalBox", ["ext-base", "css!ModalBox", "ModalUtils",], function(Ext) {
	/**
	 * ############ ###### ###### # #########.
	 * @private
	 * @type {Number}
	...
	function show(config, onCloseCallback, scope) {
		...
		const modalContainer = Terrasoft.ModalUtils.getModalContainer();
		if (!modalContainer) {
			return;
		}
		const modalContainerElement = modalContainer?.dom;
		modalContainer.addCls(openedModalBoxCssClass);
		keyMap =  new Ext.util.KeyMap(modalContainer, [{
		...
		Ext.DomHelper.insertHtml("beforeEnd", modalContainerElement, modalBoxHtml);
