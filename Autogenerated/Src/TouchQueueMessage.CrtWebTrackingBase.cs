namespace Terrasoft.Configuration
{
	...
	public class TouchQueueMessage : BaseTaskQueueMessage
	{
		...
		/// <summary>
		/// Constructor for the <see cref="TouchQueueMessage"/>.
		/// </summary>
		public TouchQueueMessage() {
		...
		public static TouchQueueMessage Create(Guid messageId, string json, int retriesCount) {
			...
			var result = Json.Deserialize<TouchQueueMessage>(json);
			result.Id = messageId;
		...
		/// <summary>
		/// Executes queue message.
		/// By default it throws not supported exception to show that message is of the base type
		/// and something went wrong or the old message type is processed.
		/// This exception type is checked by the <see cref="TouchQueueManager.ExecuteMessage(TouchQueueMessage)"./>
		/// </summary>
		...
		public override void Execute(UserConnection userConnection) {
			throw new NotImplementedException("Message type was not specified.");
		}
