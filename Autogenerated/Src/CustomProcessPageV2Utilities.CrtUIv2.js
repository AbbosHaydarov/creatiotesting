define("CustomProcessPageV2Utilities", [
		"ext-base", "terrasoft", "ProcessHelper", "MaskHelper", "ProcessModuleUtilities", "ModuleUtils"
	...
	Ext.define("Terrasoft.configuration.mixins.CustomProcessPageV2Utilities", {
		 ...
		 * @private
		 */
		_getIsLookupColumn: function(name) {
			const dataValueType = this.columns[name]?.dataValueType;
		 ...
		 * @private
		 */
		_createTypeValueQuery: function(rootSchemaName, typeAttributeName, value) {
			const esq = Ext.create("Terrasoft.EntitySchemaQuery", {
				rootSchemaName: rootSchemaName
			});
			esq.enablePrimaryColumnFilter(value);
			esq.addColumn(typeAttributeName);
			return esq;
		},
		 ...
		 * @private
		 */
		_addTypeBatchQuery: function(batch, queryAttributes, name, value) {
			const rootSchemaName = this.columns[name].referenceSchemaName;
			const typeAttributeName = Terrasoft.ModuleUtils.getTypeColumn(rootSchemaName)?.path;
			if (value[typeAttributeName]) {
				return true;
			}
			if (typeAttributeName) {
				const key = value.value;
				const esq = this._createTypeValueQuery(rootSchemaName, typeAttributeName, key);
				const queryName = `${name}_${typeAttributeName}_${key}`
				queryAttributes[queryName] = {
					attributeName: name,
					typeAttributeName: typeAttributeName
				};
				batch.addNamedQuery(esq, queryName, Terrasoft.emptyFn);
			}
		},
		 ...
		 * @private
		 */
		_getTypeBatchQuery: function(queryAttributes) {
			const lookupAttributes = Object.entries(this.values)
				.filter(([name, _]) => this._getIsLookupColumn(name))
				.filter(([_, value]) => value?.value)
				.map(([name, value]) => ({ name: name, value: value }));
			const batch = Ext.create("Terrasoft.BatchQuery");
			Terrasoft.each(lookupAttributes, ({ name, value }) => {
				this._addTypeBatchQuery(batch, queryAttributes, name, value);
			}, this);
			return batch;
		},
		 ...
		 * @private
		 */
		_processQueryResult: function(queryAttributes, response, batch) {
			Terrasoft.each(batch.queriesMap, function(index, queryName) {
				const { attributeName, typeAttributeName } = queryAttributes[queryName];
				const row = response.queryResults[index]?.rows?.[0];
				if (!row) {
					return true;
				}
				const typeAttributeValue = row[typeAttributeName]?.value;
		 ...
		 * @protected
		 */
		loadLookupAttributeTypeValues: function(callback, scope) {
			const queryAttributes = {};
			const batch = this._getTypeBatchQuery(queryAttributes);
			...
			batch.execute(function(response) {
				if (response && response.success) {
					this._processQueryResult(queryAttributes, response, batch);
				}
				Ext.callback(callback, scope);
			}, this);
		}
