define("BaseSearchRowSchema", ["NetworkUtilities", "ConfigurationEnums", "EmailHelper", "GlobalSearchViewGenerator",
	"MiniPageUtilities", "NavigationServiceUtility"], function(NetworkUtilities, ConfigurationEnums, EmailHelper) {
	return {
		...
		methods: {
			...
			/**
			 * @inheritdoc Terrasoft.BaseSchemaViewModel#getLinkUrl
			 * @overridden
			 */
			getLinkUrl: function(columnName) {
				const schemaName = this.getReferenceSchemaName(columnName);
				const id = this.get(columnName)?.value;
				let url;
				if (schemaName && id) {
					url = Terrasoft.NavigationServiceUtility.getEntitySchemaRecordUrl(schemaName, id);
				}
				return url || this.callParent(arguments);
			},
		},
		diff: /**SCHEMA_DIFF*/[
			{
