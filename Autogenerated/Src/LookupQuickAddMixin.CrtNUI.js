define("LookupQuickAddMixin", ["LookupQuickAddMixinResources", "RightUtilities", "LookupUtilities", "MaskHelper",
	...
	Ext.define("Terrasoft.configuration.mixins.LookupQuickAddMixin", {
		...
		_getCanCreateEntityOrOpenCard: function(canAdd, refSchemaName) {
			...
			const isSysModuleOrHasAddPage = !!(canAdd?.isSysModule || canAdd?.hasAddPage);
			const hasCanAddOption = !Ext.isEmpty(canAdd?.canAdd);
			return this.canManageLookups || isSysModuleOrHasAddPage || hasCanAddOption;
		},
		...
		_checkOperationToLookupSchema: function(schemaName, operation, callback) {
			Terrasoft.chain(
				...
				function(next, isSysModule) {
					this.checkIsSysModuleOrHasAddPage(schemaName, isSysModule, next);
				},
				function(next, isSysModule, hasAddPage) {
					if (isSysModule) {
		...
		/**
		 * Check whether the object has a registered section or page for adding a record.
		 * @protected
		 * @param {String} schemaName Name of schema.
		 * @param {Boolean} isSysModule Indicates whether there is a SysModule.
		 * @param {Function} callback Callback function.
		 */
		checkIsSysModuleOrHasAddPage: function(schemaName, isSysModule, callback, scope) {
			const entitySctructure = Terrasoft.configuration.EntityStructure[schemaName];
			const hasAddPage = !!entitySctructure?.pages?.some((page) => !!page.actions?.add || page.isDefault);
			Ext.callback(callback, scope, [isSysModule, hasAddPage]);
		},
		 ...
		 */
		_getOpenCardSandboxId: function(newEntityConfig, operation) {
			const columnSchemaName = this.getLookupEntitySchemaName(newEntityConfig, newEntityConfig.columnName);
			const cardSchemaName = this.getCardSchemaName(columnSchemaName, newEntityConfig.columnName, [], operation);
		...
		_getNewEntityPageConfig: function(newEntityInfo) {
			const operation = Terrasoft.ConfigurationEnums.CardOperation.ADD;
			const defValues = this._getNewEntityDefValues(newEntityInfo);
			const openModuleId = this._getOpenCardSandboxId(newEntityInfo, operation);
