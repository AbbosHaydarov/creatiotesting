		...
		public EntityCollection GetChatsByOperator(Guid operatorId, bool applyColumns = true) {
			...
			if (cacheOperator != null && cacheOperator.Active && cacheOperator.DirectedChatsCount > 0) {
				...
				if (chatsWithOperatorsCount > 0) {
					...
					List<Guid> wrongDirectedChats = directedChats.Where(chat => chat.GetColumnValue("OperatorId") != null).Select(dc => dc.PrimaryColumnValue).ToList();
					wrongDirectedChats.ForEach(chatId => ChatOperatorCache.RemoveDirectedChat(chatId));
