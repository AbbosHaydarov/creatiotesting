			...
			*/
			...
			getDisableTotpMenuItem: function() {
				return this.getButtonMenuItem({
					...
					"Click": {"bindTo": "confirmDisconnectAuthApp"},
			...
			disableTotpForUser: function() {
				...
				var enabledTwoFa = Terrasoft.Features.getIsEnabled("Enabled2FAFlow")
					&& Terrasoft.Features.getIsEnabled("UseSeparatedPage2FA");
				let urlPrefix = Terrasoft.workspaceBaseUrl;
				let serviceUrl = enabledTwoFa
					? `${urlPrefix}/api/Totp/Authenticator/Disconnect`
				...
				Terrasoft.AjaxProvider.request({
					...
					callback: function(options, result, response) {
						...
						var disconnectResult = responseObject;
						if (disconnectResult && !enabledTwoFa)  {
							disconnectResult = disconnectResult.ResetTotpResult;
						}
						if (disconnectResult && disconnectResult.isSuccess) {
							this.showInformationDialog(this.get("Resources.Strings.Reset2FAIsSuccessful"));
						} else {
							this.log(disconnectResult.message);
							this.showInformationDialog(disconnectResult.message);
						}
					},
					scope: this
				});
			...
			confirmDisconnectAuthApp: function(result) {
				if (result) {
					this.showInformationDialog(resources.localizableStrings[result]);
					return;
				}
				this.getDisconnectAuthAppMessage(function(message){
					this.showConfirmationDialog(message, function(returnCode) {
						this.onDisconnect(returnCode);
					}, [Terrasoft.MessageBoxButtons.YES, Terrasoft.MessageBoxButtons.NO], null);
				}, this);
			},

			getDisconnectAuthAppMessage: function(callback, scope) {
				const message = this.get("Resources.Strings.DisconnectAuthAppMessage");
				Ext.callback(callback, scope || this, [message]);
			},

			onDisconnect: function(returnCode) {
				if (returnCode !== this.Terrasoft.MessageBoxButtons.YES.returnCode) {
					return;
				} else {
					this.disableTotpForUser();
				}
			},
			...
			setTotpStatus: function() {
				var serviceUrl = Terrasoft.Features.getIsEnabled("Enabled2FAFlow")
					? '/../ServiceModel/AuthService.svc/GetAuthenticationSettings'
					: '/../ServiceModel/TotpSetupService.svc/GetTotpInfo';
				Terrasoft.AjaxProvider.request({
					url: Terrasoft.workspaceBaseUrl + serviceUrl,
					method: "POST",
					jsonData: null,
					callback: function(options, result, response) {
						var responseObject = Terrasoft.decode(response.responseText);
						if (Terrasoft.Features.getIsEnabled("Enabled2FAFlow")) {
							var isTotpEnabled = false;
							if (responseObject.enabledTwoFactorAuthFlows) {
								isTotpEnabled = responseObject.enabledTwoFactorAuthFlows.map((x) => x.code).includes(
									'TOTP',
								);
							}
							this.set("IsTotpEnabled", isTotpEnabled);
						} else {
