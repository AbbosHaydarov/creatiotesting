namespace Terrasoft.Configuration
{
	...
	public partial class ContactCommunication_CrtBulkEmailEngineEventsProcess<TEntity>
	{
		...
		public virtual void UpdateEmailValue(Guid contactId) {
			...
			contactESQ.AddColumn("IsNonActualEmail");
			var contact = contactESQ.GetEntity(UserConnection, contactId)
				?? throw new NullReferenceException($"Contact {contactId} was not found.");
			var currentValue = contact.GetTypedColumnValue<string>("Email");
			var currentState = contact.GetTypedColumnValue<bool>("IsNonActualEmail");
			KeyValuePair<string, bool> actualData = GetActualContactEmail(contactId);
			if (currentValue != actualData.Key || currentState != actualData.Value) {
				var columnValues = new Dictionary<string, object> {
					{ "Email", actualData.Key },
					{ "IsNonActualEmail", actualData.Value }
				};
		...
		public virtual KeyValuePair<string, bool> GetActualContactEmail(Guid contactId) {
			...
			esq.AddColumn("Primary").OrderByDesc(2);
			esq.AddColumn("CreatedOn").OrderByDesc(3);
