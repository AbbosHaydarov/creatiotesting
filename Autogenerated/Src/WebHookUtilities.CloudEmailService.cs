namespace Terrasoft.Configuration
{
	...
	public static class WebHookUtilities
	{
		...
		private static Select AppendWhereClauseByHash(Guid bulkEmailId, string preparedUrl, Select rIdSelect) {
			Guid urlHash = MailingUtilities.GetMD5HashGuid(preparedUrl.ToLower());
			rIdSelect = rIdSelect.Where("Hash").IsEqual(Column.Parameter(urlHash)).And("BulkEmailId")
				.IsEqual(Column.Parameter(bulkEmailId)) as Select;
			return rIdSelect;
		}

		private static Select AppendWhereCloseByTrackingParams(Guid bulkEmailId, int trackId, long replicaMask,
			Select rIdSelect) {
			rIdSelect = rIdSelect.Where("BpmTrackId").IsEqual(Column.Parameter(trackId)).And("BpmReplicaMask")
				.IsEqual(Column.Parameter(replicaMask)).And("BulkEmailId")
				.IsEqual(Column.Parameter(bulkEmailId)) as Select;
			return rIdSelect;
		}

		private static Select BuildRIdSelect(UserConnection userConnection, Guid bulkEmailId, string preparedUrl) {
			Dictionary<string, string> uriTrackingParams =
				BulkEmailUriHelper.GetParametersFromUri(preparedUrl, UrlTrackIdParameterName,
					UrlReplicaMaskParameterName);
			Select rIdSelect = new Select(userConnection).Column("RId").From("BulkEmailHyperlink");
		...
		private static IEnumerable<KeyValuePair<IEnumerable<Guid>, BaseWebHook>> FilterWebHooksByType(Type type,
			...
			IEnumerable<KeyValuePair<Guid, IEnumerable<BaseWebHook>>> list =
				webHookList.Where(webHook => webHook.Value.All(w => w.GetType() == type));
			IEnumerable<KeyValuePair<Guid, BaseWebHook>> fullWebHookMap =
				list.SelectMany(x => x.Value.Select(n => new KeyValuePair<Guid, BaseWebHook>(x.Key, n)));
			try {
				IEnumerable<KeyValuePair<IEnumerable<Guid>, BaseWebHook>> lastWebHooks = fullWebHookMap
					.GroupBy(webHook => webHook.Value.GetGroupKey()).Select(group =>
		...
		private static Select GetEmailForActualizationFromMandrillRecipientSelect(UserConnection userConnection,
			...
			DateTime oldEmailDate = DateTime.Today.AddDays(-statisticUpdatePeriod);
			var bulkEmailSelect = new Select(userConnection).Column("be", "Id").Column("be", "CategoryId")
				.From("BulkEmail").As("be").Where("be", "CategoryId")
				.IsEqual(Column.Parameter(MailingConsts.MassmailingBulkEmailCategoryId)).And("be", "SendStartDate")
				.IsLess(Column.Parameter(oldEmailDate)).And("be", "StatusId").In(
			...
			var mandrillRecipientSelect = new Select(userConnection).Column(Column.Parameter(1))
				.From("MandrillRecipient").As("mr").Where("mr", "BulkEmailRId").IsEqual("be", "RId") as Select;
		...
		private static Select GetEmailForActualizationSelect(UserConnection userConnection,
			...
			var emailsIdsSelect = new Select(userConnection).Column("be", "Id").Column("be", "CategoryId")
				.From("BulkEmail").As("be").InnerJoin("MandrillMessageResponse").As("mmr")
				.On("be", bulkEmailJoinProperty).IsEqual("mmr", mandrillMessageResponseJoinProperty)
				.Where("be", "ResponseProcessingCompleted").IsEqual(Column.Parameter(false)).And("be", "StatusId").In(
					Column.Parameter(MarketingConsts.BulkEmailStatusFinishedId),
					Column.Parameter(MarketingConsts.BulkEmailStatusStoppedId),
					Column.Parameter(MarketingConsts.BulkEmailStatusActiveId)) as Select;
			emailsIdsSelect.SpecifyNoLockHints();
		...
		private static Dictionary<Guid, Guid> GetEmailsForActualization(UserConnection userConnections,
			...
			Select getByIdsSelect = GetEmailForActualizationSelect(userConnections, "Id", "BulkEmailId");
			Select getByRIdsSelect = GetEmailForActualizationSelect(userConnections, "RId", "BulkEmailRId");
			Select bulkEmailIds =
				GetEmailForActualizationFromMandrillRecipientSelect(userConnections, statisticUpdatePeriod);
		...
		private static Type GetWebHookType(string webHookJson) {
			var jsonData = Json.Deserialize(webHookJson) as JObject;
			if (jsonData == null) {
				return null;
			}
		...
		private static string RemoveExtraParametersFromUri(string hyperLink) {
			string preparedUrl = HttpUtility.UrlDecode(hyperLink);
		...
		public static void ActualizeMailingRecipients(UserConnection userConnection) {
			...
			int packageSizeForTransferAudienceToBulkEmailTarget = CoreConf.SysSettings.GetValue(userConnection,
		...
		/// <summary>
		/// Creates upper function filter, which compares column value in uppercase with value provided in uppercase
		/// </summary>
		/// <param name="esq">Instance of entity schema query.</param>
		/// <param name="columnName">Name of the column in entity schema query.</param>
		/// <param name="filterValue">Value to search for </param>
		/// <param name="filterComparisonType">Comparison type filter </param>
		public static EntitySchemaQueryFilter CreateUpperFunctionFilter(EntitySchemaQuery esq, string columnName,
			string filterValue, FilterComparisonType filterComparisonType = FilterComparisonType.Equal) {
			EntitySchemaQueryFunction upperNameFunction = esq.CreateUpperFunction(columnName);
			var filter = new EntitySchemaQueryFilter(filterComparisonType) {
		...
		[Obsolete(
			"7.14.4|Use FilterUnsubscribeWebHook(UserConnection userConnection, IEnumerable<KeyValuePair<IEnumerable<Guid>, BaseWebHook>> lastWebHooks)")]
		public static List<BaseWebHook> FilterUnsubscribeWebHook(UserConnection userConnection,
			List<BaseWebHook> lastWebHooks) {
			var unsubscribeFromAllMailings =
				(bool)CoreConf.SysSettings.GetValue(userConnection, "UnsubscribeFromAllMailings");
			return !unsubscribeFromAllMailings ? lastWebHooks.Where(webHook => !(webHook is Unsubscribe)).ToList()
				: lastWebHooks;
		}
		...
		/// <param name="lastWebHooks">Collection of webhooks.</param>
		/// <returns>Filtered collection of webhooks.</returns>
		public static IEnumerable<KeyValuePair<IEnumerable<Guid>, BaseWebHook>> FilterUnsubscribeWebHook(
			UserConnection userConnection, IEnumerable<KeyValuePair<IEnumerable<Guid>, BaseWebHook>> lastWebHooks) {
			var unsubscribeFromAllMailings = (bool)CoreConf.SysSettings.GetValue(userConnection,
				"UnsubscribeFromAllMailings");
			return !unsubscribeFromAllMailings ? lastWebHooks.Where(webHook => !(webHook.Value is Unsubscribe))
				: lastWebHooks;
		}
		...
		public static int GetHyperLinkRId(UserConnection userConnection, string hyperLink, Guid bulkEmailId) {
			string preparedUrl = RemoveExtraParametersFromUri(hyperLink);
			Select rIdSelect = BuildRIdSelect(userConnection, bulkEmailId, preparedUrl);
		...
		public static List<BaseWebHook> GetLastWebHooks(UserConnection userConnection, List<JArray> jArrayList) {
			...
			var lastWebHooks = new List<BaseWebHook>();
