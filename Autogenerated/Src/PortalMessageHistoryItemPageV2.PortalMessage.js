		...
		return {
			...
			methods: {
				...
				*/
				...
				/**
				 * Replaces portal file links to secure portal service url. 
				 * @protected
				 * @param value Bound message.
				 * @returns {String} Processed message.
				 */
				replaceLinksToPortalFileService: function(message){
					const portalFileServiceUrl =  "/ssp/rest/PortalMessageFileService/GetPortalMessageFile/" + this.get("HistorySchemaName") + "/" + this.get("Id")
					const replacedText = message.replace(/(?<=<img[^>]+src\s*=\s*["']\.\.)(\/rest\/FileService\/GetFile\/[^/]+)/gmi, portalFileServiceUrl);
					return replacedText;
				},
				...
				validatePortalMessage: function(message) {
					if (message && this.Ext.isString(message)) {
						...
						if (Terrasoft.isCurrentUserSsp() && Terrasoft.Features.getIsEnabled("SecurePortalMessageFileInHistory")) {
							message = this.replaceLinksToPortalFileService(message);
						}
						return message;
					}
					return message;
