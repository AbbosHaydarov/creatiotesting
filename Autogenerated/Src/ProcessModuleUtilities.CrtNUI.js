define("ProcessModuleUtilities", [
	...
	...(Terrasoft.UseMfForAngularElements && (!Terrasoft.isAngularHost || Terrasoft.UseMfForAngularElements_8_1_3 && !Terrasoft.SwitchOffErrorListDialogMf)
		? ["mf!ErrorListDialogComponent", "css-ltr!ErrorListDialogComponentStyles"]
		: ["ErrorListDialogComponentOld"])
], function(resources, ConfigurationConstants, ConfigurationEnums, DesktopPopupNotification,
			LookupUtilities, ServiceHelper, devkit) {
	...
	Ext.define("Terrasoft.configuration.ProcessModuleUtilities", {
		...
		_showSuccessfullyRunProcessPopup: function(processCaption) {
			const crtZoneService = new devkit.CrtZoneService();
			crtZoneService.runOutside(() => {
		 ...
		 * @private
		 */
		_loadAvailablePackagesByESQ: function(list) {
			const esq = Ext.create("Terrasoft.EntitySchemaQuery", {
			...
			esq.getEntityCollection(function(response) {
				...
				this.tryAddCustomPackage.call(this, list, rows, this._sortList);
			}, this);
		},
		 ...
		 * @private
		 */
		_loadAvailablePackages: function(list) {
			let request = Ext.create("Terrasoft.GetAvailablePackagesRequest");
			request.execute(function(response) {
				if (response && response.success && response.availablePackages) {
					const rows = {};
					Terrasoft.each(response.availablePackages, function(value, uId) {
						const listValue = {
							UId: uId,
							value: uId,
							displayValue: value
		 ...
		 * @private
		 */
		_sortList: function(innerList, innerRows) {
		...
		onPrepareSysPackageList: function(filter, list) {
			list.clear();
			if (Terrasoft.Features.getIsEnabled("AppsFeatures.CreateAppVirtualCurrentPackage")) {
				this._loadAvailablePackages(list);
			} else {
				this._loadAvailablePackagesByESQ(list);
			}
		},
		...
		_runProcessForMultipleRecords: function(config, selectionConfig) {
			...
			request.execute(function(response) {
				...
				if (response.success === false) {
					Terrasoft.showErrorMessage(response.errorInfo.message ?? "Failed to start process");
				} else {
					this._showSuccessfullyRunProcessPopup(config.processSchemaCaption);
				}
				Ext.callback(config.callback, config.scope, [response.success]);
			}, this);
		},
