		...
		return {
			...
			methods: {
				...
				addEmailRelation: function(columnName) {
					...
					var entitySchemaName = this.getSchemaNameByRelationColumnName(columnName);
					var schemaModuleStructure = moduleStructure[entitySchemaName];
					var schemaPages = schemaModuleStructure.pages;
					var cardSchema = schemaModuleStructure.cardSchema;
					var defaultValues = this.getRelationValuePairs(columnName, entitySchemaName);
					if (this.Ext.isArray(schemaPages) && this.getIsFeatureDisabled("IgnoreTypePagesInEmailPanel")) {
						var cardInfo = this.Terrasoft.findWhere(schemaPages, {name: entitySchemaName});
					...
					if (!cardSchema) {
						const entityStructure = Terrasoft.ModuleUtils.getEntityStructureByName(entitySchemaName);
