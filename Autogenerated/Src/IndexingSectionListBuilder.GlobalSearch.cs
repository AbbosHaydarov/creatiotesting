namespace Terrasoft.Configuration.GlobalSearch
{
	...
	public class IndexingSectionListBuilder : IndexingEntityListBuilder
	{
		...
		private static readonly string _sysFileSchemaName = "SysFile";
		private static readonly string _sysFileRecordIdColumnName = "RecordId";
		...
		private void AddFileSchemaQuery(EntitySchema entitySchema, List<EntitySchemaQuery> esqList) {
			...
			AddFileTypeFilter(fileEsq);
			esqList.Add(fileEsq);
		}

		private void AddSysFileSchemaQuery(List<EntitySchemaQuery> esqList) {
			var sysFileSchema = _userConnection.EntitySchemaManager.FindInstanceByName(_sysFileSchemaName);
			if (sysFileSchema == null || sysFileSchema.Columns.FindByName("Type") == null) {
				return;
			}
			if (IgnoredEntityList.GetIsIgnoredSection(sysFileSchema)) {
				return;
			}
			var fileEsq = GetSelectForIndexation(sysFileSchema);
			var recordIdColumn = sysFileSchema.Columns.FindByName(_sysFileRecordIdColumnName);
			if (recordIdColumn == null) {
				return;
			}
			var recordIdColumnAlias = GlobalSearchColumnUtils.GetPrimaryColumnAlias(recordIdColumn, _sysFileSchemaName);
			fileEsq.AddColumn(_sysFileRecordIdColumnName).SetForcedQueryColumnValueAlias($"##{recordIdColumnAlias}##");
			AddFileTypeFilter(fileEsq);
			esqList.Add(fileEsq);
		}

		private void AddFileTypeFilter(EntitySchemaQuery esq) {
			var filter = esq.CreateFilterWithParameters(FilterComparisonType.NotEqual, "Type", _fileEntityLinkTypeId);
		...
		protected List<EntitySchemaQuery> GetEntitySchemaQueries(List<Guid> entityUIds) {
			...
			AddSysFileSchemaQuery(entitySchemaQueries);
