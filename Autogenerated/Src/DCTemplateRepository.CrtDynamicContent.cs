namespace Terrasoft.Configuration.DynamicContent
{
	...
	public class DCTemplateRepository<T> where T : DCTemplateModel, new()
	{
		...
		private void DeleteBlocksInReplicas(Guid templateId) {
			var replicaIdSelect = new Select(UserConnection)
				.Column("Id")
				.From("DCReplica")
				.Where("DCTemplateId").IsEqual(Column.Parameter(templateId)) as Select;
			new Delete(UserConnection)
				.From("DCBlockInReplica")
				.Where("DCReplicaId").In(replicaIdSelect)
				.Execute();
		}
		...
		protected virtual bool DeleteTemplate(Guid templateId) {
			...
			if (template.FetchFromDB(templateId)) {
				DeleteBlocksInReplicas(templateId);
