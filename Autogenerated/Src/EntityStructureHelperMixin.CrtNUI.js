define("EntityStructureHelperMixin", ["ext-base", "terrasoft", "ConfigurationEnums",
	"EntityStructureHelperMixinResources", "ServiceHelper", "EntityStructureHelper", "ViewGeneratorV2"
], function(Ext, Terrasoft, ConfigurationEnums, resources, ServiceHelper, EntityStructureHelper) {
	...
	Ext.define("Terrasoft.configuration.mixins.EntityStructureHelperMixin", {
		...
		getBackwardColumns: function(entitySchemaName, callback, scope) {
			...
			select.addColumn("SysSchema.SysPackage.HierarchyLevel", "HierarchyLevel");
			...
			select.getEntityCollection(function(response) {
				if (response && response.success && response.collection) {
					var items = EntityStructureHelper.getProcessedBackwardColumnItems(response.collection.getItems());
