...
define("BulkEmailTemplateValidator", ["BulkEmailTemplateValidatorResources", "AcademyUtilities",
		...
		"MacroUtils","EmailHelperModule", "ContentBuilderConstants", "DynamicContentValidator",
		"BulkEmailSenderValidator", "ContentExporterFactory"],
	function(resources, AcademyUtilities, ReplicaBuilder, BulkEmailTemplateHelper, BulkEmailHelper, MarketingEnums,
		...
		Ext.define("Terrasoft.configuration.BulkEmailTemplateValidator", {
			 ...
			 * @private
			 */
			_getInvalidImageFormatMessage: function(validationLevel, itemTypes) {
				var elementNames = resources.localizableStrings.MjmlElementNames.split(",").reduce(function(names, valueStr) {
					var value = valueStr.split(":");
					names[value[0]] = value[1];
					return names;
				}, {});
				var description = Object.entries(itemTypes).reduce(function(message, value) {
					var count = value[1] > 1 ? value[1] + " " : "";
					var elementName = elementNames.hasOwnProperty(value[0])
						? elementNames[value[0]]
						: resources.localizableStrings.UnknownElementName;
					elementName = value[1] > 1 ? elementName.replace("(s)","s") : elementName.replace("(s)","");
					return message += count + elementName + ", ";
				}, "");
				description = description.slice(0,-2);
				return {
					Caption: resources.localizableStrings.MsgInvalidImageFormatCaption,
					Description: Ext.String.format(resources.localizableStrings.MsgInvalidImageFormat, description),
					Level: validationLevel
				};
			},
			/**
			...
			_validateEmbeddedImageFormat: function(itemConfig) {
				switch (itemConfig.ItemType) {
					case Terrasoft.ContentBuilderBodyItemType.mjblock.value:
						return this._validateImageSource(itemConfig.Styles["background-image"]);
					case Terrasoft.ContentBuilderBodyItemType.mjhero.value:
						return this._validateImageSource(itemConfig.WrapperStyles["background-image"]);
					case Terrasoft.ContentBuilderBodyItemType.mjimage.value:
						return this._validateImageSource(itemConfig.ImageConfig.url);
					case Terrasoft.ContentBuilderBodyItemType.mjraw.value:
						return this._validateMacrosImage(itemConfig.Macros) && this._validateHtmlImage(itemConfig.HtmlSrc);
					default:
						return true;
				}
			},
			
			_validateMacrosImage: function(macros) {
				var isValid = true;
				Terrasoft.each(macros, function (macro) {
					isValid = macro.DataValueType !== Terrasoft.DataValueType.IMAGE || this._validateImageSource(macro.Value);
					if (!isValid) {
						return false;
					}
				}, this);
			...
			_validateHtmlImage: function(htmlSource) {
				var isValid = true;
				var images = [];
				this._findInlineImages(htmlSource, images);
				Terrasoft.each(images, function(image) {
					if (!this._validateImageSource(image)) {
						isValid = false;
						return false;
					}
				}, this);
			...
			_validateImageSource: function(data) {
				if (!data) {
					return true;
				}
				var regex = /data:([a-z-]+)\/([^;]+);base64[^"|')]+/;
				if (regex.test(data)) {
			...
			initValidationTypes: function() {
				...
				validationTypes[Terrasoft.BulkEmailValidationType.INVALID_IMAGE_FORMAT] = {
					level: Terrasoft.BulkEmailValidationLevel.ERROR,
					getMessage: this._getInvalidImageFormatMessage,
					runtimeAction: null
				};
			...
			/**
			 * Validates format of embeded images.
			 * @protected
			 * @param {Object} templateConfig Template config.
			 * @param {Function} callback Callback function.
			 * @param {Object} scope Context.
			 */
			validateEmbededImages: function(templateConfig, callback, scope) {
				var invalidItemTypes = {};
				this.validateImagesFormat(templateConfig, invalidItemTypes);
				if (!Object.keys(invalidItemTypes).length) {
					callback.call(this);
				} else {
					var validationType = Terrasoft.BulkEmailValidationType.INVALID_IMAGE_FORMAT;
					this._routeValidationMessage(validationType, invalidItemTypes, callback);
				}
			},
			 ...
			 * Validates format of embeded images.
			 * @protected
			 * @param {Object} config Template item config.
			 * @param {Object} itemTypes Types of invalid items.
			 */
			validateImagesFormat: function(config, itemTypes) {
				var isValid = this._validateEmbeddedImageFormat(config)
				if (!isValid) {
					if (!itemTypes.hasOwnProperty(config.ItemType)) {
						itemTypes[config.ItemType] = 1;
					} else {
						itemTypes[config.ItemType]++;
					}
				}
				Terrasoft.each(config.Items, function (itemConfig) {
					itemType = this.validateImagesFormat(itemConfig, itemTypes);
				}, this);
			},
			...
			validateErrors: function(config, callback, unsubscribeFixAction, scope) {
				Terrasoft.chain(
					...
					function(next) {
						this.validateEmbededImages(config.templateConfig, next);
					},
					function(next) {
