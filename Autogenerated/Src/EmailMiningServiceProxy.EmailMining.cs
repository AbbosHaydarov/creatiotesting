namespace Terrasoft.Configuration.EmailMining
{
	...
	public class EmailMiningServiceProxy : IEmailMiningServiceProxy
	{
		...
		public EmailMiningServiceProxy(UserConnection userConnection) {
			...
			if (!HttpRequestClientFactory.TryGetNamedInstance("EmailMiningServiceProxyHttpClient", out _httpClient)) {
		...
		public Dictionary<Entity, GetEmailEntitiesResponse> BatchCall(IEnumerable<Entity> emails) {
			...
			IHttpResponse batchResponse;
			try {
				HttpRequestConfig requestConfig = new HttpRequestConfig(new Uri(serviceUrl), ServiceEmailParsingMethod) {
					Body = requests,
					RequestTimeout = (int)_requestTimeout.TotalMilliseconds
				}.WithOAuth<EnrichmentFeatures.UseOAuth>(IdentityServiceWrapper, EnrichmentConstants.EnrichmentOAuthScope);
				requestConfig.Headers.Add("ApiKey", apiKey);
				batchResponse = _httpClient.SendWithJsonBody(requestConfig);
			} catch (UriFormatException e) {
				throw new IncorrectConfigurationException(EmailMiningConsts.TextParsingServiceSysSetting,
					serviceUrl, e.Message);
			}
			if (batchResponse.Exception is JsonException) {
				throw new IncorrectConfigurationException(EmailMiningConsts.TextParsingServiceSysSetting,
			...
			if (batchResponseStatusCode >= 400 || batchResponseStatusCode == 0) {
				...
				switch (batchResponse.StatusCode) {
					case 0:
						_log.ErrorFormat("Request cannot be completed by Http client. Error: {0}",
					...
					default:
						_log.ErrorFormat("Cannot process email pack. Response code: {0}({1}). Error: {2}",
							batchResponse.StatusCode, (int)batchResponse.StatusCode, batchResponse.Content);
						return result;
				}
			} else {
