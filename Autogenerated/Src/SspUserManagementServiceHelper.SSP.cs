	...
	public class SspUserManagementServiceHelper
	{
		...
		private Select GetSspLicPackageSelectQuery() {
			return new Select(UserConnection)
					...
					.And()
						.OpenBlock("licPackage", "IsSsp").IsEqual(Column.Parameter(true))
							.Or("licPackage", "Operations").IsLike(Column.Parameter("%ExternalB2CLogin%"))
							.Or("licPackage", "Operations").IsLike(Column.Parameter("%SelfServicePortalLogin%"))
