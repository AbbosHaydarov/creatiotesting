 namespace Terrasoft.Sync.Exchange
{
	...
	public class ExchangeTask : ExchangeBase
	{
		...
		private DateTime? GetExchangeTaskDueDate (Exchange.Task exchangeTask, UserConnection userConnection) {
			if (userConnection.GetIsFeatureEnabled("OriginalDueDateForCompletedExchangeTask")) {
				return exchangeTask.DueDate;
			} 
		...
		protected virtual void SetActivityProperties(Entity activity, Exchange.Task exchangeTask, SyncContext context,
			...
			if (!activity.IsColumnValueLoaded("StatusId") ||
					activity.GetTypedColumnValue<Guid>("StatusId") != ActivityConsts.CanceledStatusUId) {
		...
		/// </summary>
		/// <param name="uc">User connection.</param>
		...
		/// </remarks>
		public void SetStartAndDueDate(UserConnection uc, Exchange.Task exchangeTask,
			out DateTime startDate, out DateTime dueDate) {
			DateTime? exchangeTaskStartDate = InitExchangeTaskDate(exchangeTask.StartDate, uc);
			DateTime? exchangeTaskDueDate = InitExchangeTaskDate(GetExchangeTaskDueDate(exchangeTask, uc), uc);
			if (exchangeTaskStartDate == null) {
				if (exchangeTaskDueDate == null) {
					startDate = DateTime.Now.Date;
				} else {
					if (uc.GetIsFeatureEnabled("FullDueDayTask")) {
						startDate = exchangeTaskDueDate.Value.Date;
						dueDate = startDate.AddDays(1).AddMinutes(-1);
						return;
					}
