 namespace Terrasoft.Configuration
{
	...
	public class ExchangeUtilityImpl: IExchangeUtility
	{
		...
		public static readonly Exchange.ExtendedPropertyDefinition RemoteIdProperty = new Exchange.ExtendedPropertyDefinition(
			Exchange.DefaultExtendedPropertySet.PublicStrings, "RemoteId", Exchange.MapiPropertyType.String);
		...
		private ExchangeCredentials CreateExchangeCredentials(UserConnection userConnection,
			...
			var mailbox = mailboxService.GetMailboxBySenderEmailAddress(emailAddress, !ignoreRights);
			var synchCreds = mailbox.ConvertToSynchronizationCredentials(userConnection, true);
			var credentials = new ExchangeCredentials {
				UserName = synchCreds.UserName,
				UserPassword = synchCreds.Password,
				IsAutodiscover = synchCreds.IsAutodiscover,
				ServerAddress = synchCreds.ServiceUrl,
				UseOAuth = synchCreds.UseOAuth,
				UseImpersonation = synchCreds.FeatureValues["UseImpersonation"]
			};
			if (credentials.UseOAuth) {
				credentials.Token = synchCreds.AccessToken;
			}
			return credentials;
		}
		...
		private string GetToken(UserConnection userConnection, string senderEmailAddress) {
			...
			string token = OAuthUtils.RefreshAccessToken(oauthClassName, senderEmailAddress, userConnection);
			return token;
		}
		...
		private void ProcessingItemBindException(Exception error, Exchange.ItemId id) {
			...
			if (error is ArgumentOutOfRangeException || baseException is ArgumentOutOfRangeException) {
				Log.ErrorFormat("[ExchangeUtility.SafeBindItem]: Error while loading item with Id: {0}", error, id);
				return;
			}
			if (!(baseException is Exchange.ServiceResponseException)) {
				throw error;
		...
		private void UnlockEntityForSync(SyncContext context, IRemoteItem remoteItem, LocalItem localItem) {
			var helper = ClassFactory.Get<EntitySynchronizerHelper>();
			helper.UnlockEntities(new List<string> { remoteItem.Id }, context.UserConnection.CurrentUser.ContactId,
				"Exchange integration", context.UserConnection);
		}

		private void ConfigureImpersonation(string senderEmailAddress,
				Exchange.ExchangeService exchangeService, ExchangeCredentials credentials) {
			if (credentials.UseImpersonation) {
				exchangeService.ImpersonatedUserId =
					new Exchange.ImpersonatedUserId(Exchange.ConnectingIdType.SmtpAddress, senderEmailAddress);
			}
		}
		#endregion
		...
		internal Exchange.ExchangeService CreateExchangeService(UserConnection userConnection, ExchangeCredentials credentials,
			...
			try {
				...
				ConfigureImpersonation(senderEmailAddress, exchangeService, credentials);
				...
				if (credentials.IsAutodiscover) {
					exchangeService.AutodiscoverUrl(senderEmailAddress, delegate (string url) {
						var regexString = Core.Configuration.SysSettings.GetValue<string>(userConnection,
							"ExchangeAutodiscoverRedirectUrlRegex", ".*");
						return Regex.IsMatch(url, regexString, RegexOptions.IgnoreCase);
					});
				} else {
					exchangeService.Url = new Uri($"https://{credentials.ServerAddress}/EWS/Exchange.asmx");
				}
				if (credentials.UseOAuth) {
					exchangeService.Credentials = new Exchange.OAuthCredentials(credentials.Token);
				} else {
		...
		/// and returns a hierarchical collection <see cref="ExchangeMailFolder"/>.
		/// </summary>
		...
		/// <returns>Collection objects of <see cref="ExchangeMailFolder"/>.
		/// </returns>
		...
		public void SyncExchangeItems(UserConnection userConnection, string senderEmailAddress,
			...
			try {
				...
				synAgent.RemoteItemAppliedInLocalStore += UnlockEntityForSync;
				SetSecurityProtocolOptions(userConnection);
