namespace Terrasoft.Configuration
{
	...
	#region Class: MultiDeleteFeatures

	/// <summary>
	/// Multi delete functionality features.
	/// </summary>
	public static class MultiDeleteFeatures
	{

		#region Class: UseColumnUsageTracker

		/// <summary>
		/// Indicates whether to use mechanism that optimizes the loading of entity column values.
		/// </summary>
		/// <inheritdoc cref="Creatio.FeatureToggling.FeatureMetadata" />
		public class UseColumnUsageTracker : FeatureMetadata
		{

			#region Constructors: Public

			/// <summary>
			/// Initializes a new instance of the <see cref="UseColumnUsageTracker" /> type.
			/// </summary>
			public UseColumnUsageTracker() {
				IsEnabled = true;
			}
	...
	public class MultiDeleteExecutor : IJobExecutor
	{
		...
		private void PreparedParameters(IDictionary<string, object> parameters, UsageContext usageContext) {
			...
			preparedParameters[BaseRecordsOperationWorker.UsageContextParameterKey] = usageContext;
		...
		public virtual void Execute(UserConnection userConnection, IDictionary<string, object> parameters) {
			...
			PreparedParameters(parameters, usageContext);
			try {
				_log.Info("Start.");
				PreparedRecordIds();
				if (IsCascade) {
		...
		private UsageContext GetUsageContext() {
			if (Features.GetIsDisabled<MultiDeleteFeatures.UseColumnUsageTracker>()) {
				return null;
			}
			var factory = ClassFactory.Get<IUsageContextFactory>();
			return factory.Create("MultiDelete");
		}
