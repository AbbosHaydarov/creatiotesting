 namespace Terrasoft.Sync.Exchange
{
	...
	public class ExchangeAppointment : ExchangeBase
	{
		...
		private const string DefaultDueDateFormat = "dd.MM.yyyy HH:mm:ss";
		...
		private void SetSendNotification(SyncContext context, Entity activity, SyncEntity syncEntity) {
			...
			if (!((string)syncEntity.ExtraParameters).IsNullOrEmpty()) {
				...
				var rawDueDate = (string)activityExtendProperty["EndDate"];
				DateTime previousDueDate = DateTime.MinValue;
				if (!DateTime.TryParse(rawDueDate, out previousDueDate)) {
					var format = SysSettings.GetValue<string>(context.UserConnection, "AppointmentDueDateFormat", DefaultDueDateFormat);
					previousDueDate = DateTime.ParseExact(rawDueDate, format, CultureInfo.InvariantCulture);
				}
				SendNotifications = IsActivityStatusNotFinal(activity, previousStatusId, context.UserConnection) &&
						IsDueDateInFuture(activity, previousDueDate, context);
		...
		private bool IsActivityStatusNotFinal(Entity activity, Guid previousStatusId,
		...
		private bool IsDueDateInFuture(Entity activity, DateTime previousDueDate, SyncContext context) {
			DateTime currentDateTime = context.CurrentSyncStartVersion.ToLocalTime();
			return (activity.GetTypedColumnValue<DateTime>("DueDate").ToLocalTime() >= currentDateTime) ||
		...
		protected virtual bool IsCreatingDuplicateAppointment(Exchange.Appointment appointment, LocalItem localItem,
			...
			DateTime utcStartDate = DateTime.SpecifyKind(activity.GetTypedColumnValue<DateTime>("StartDate"), DateTimeKind.Unspecified);
			utcStartDate = TimeZoneInfo.ConvertTimeToUtc(utcStartDate, userConnection.CurrentUser.TimeZone);
			DateTime utcDueDate = DateTime.SpecifyKind(activity.GetTypedColumnValue<DateTime>("DueDate"), DateTimeKind.Unspecified);
			...
			Select activitySelect = new Select(userConnection)
				...
				.From("Activity")
				.Where("Title").IsEqual(Column.Parameter(activity.GetTypedColumnValue<string>("Title")))
					.And("Location").IsEqual(Column.Parameter(activity.GetTypedColumnValue<string>("Location")))
					.And("StartDate").IsEqual(Column.Parameter(utcStartDate))
					.And("DueDate").IsEqual(Column.Parameter(utcDueDate))
					.And("PriorityId").IsEqual(Column.Parameter(activity.GetTypedColumnValue<Guid>("PriorityId"))) as Select;
		...
		public void UpdateExtraParameters(string exchangeAppointmentId = null, string activityHash = null,
			...
			var activity = _activitySyncEntity.Entity;
			...
			activityExtendProperty["StatusId"] = activity.GetTypedColumnValue<Guid>("StatusId").ToString();
			activityExtendProperty["EndDate"] = activity.GetTypedColumnValue<DateTime>("DueDate").ToString();
		...
		public override void FillRemoteItem(SyncContext context, LocalItem localItem) {
			...
			if(_activitySyncEntity == null) {
				LogInfo(context, "FillRemoteItem _activitySyncEntity is empty, fill remote item skipped.");
				return;
			}
