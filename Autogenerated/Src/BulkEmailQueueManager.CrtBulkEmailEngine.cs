namespace Terrasoft.Configuration
{
	...
	public class BulkEmailQueueManager : BaseAudienceQueueManager<BulkEmailQueueMessage>
	{
		...
		protected override BulkEmailQueueMessage CreateMessage(Guid id, string json, int retries) {
			var baseMessage = BulkEmailQueueMessage.Create(id, json, retries);
			switch (baseMessage.Type) {
				case BulkEmailQueueMessageType.AddAudienceByList:
					return BulkEmailQueueMessage.Create<AddAudienceByListQueueMessage>(id, json, retries);
				case BulkEmailQueueMessageType.AddAudienceByFilter:
					return BulkEmailQueueMessage.Create<AddAudienceByFilterQueueMessage>(id, json, retries);
				case BulkEmailQueueMessageType.AddAudienceByFolderId:
					return BulkEmailQueueMessage.Create<AddAudienceByFolderQueueMessage>(id, json, retries);
				case BulkEmailQueueMessageType.RemoveAudienceByList:
					return BulkEmailQueueMessage.Create<RemoveAudienceByListQueueMessage>(id, json, retries);
				case BulkEmailQueueMessageType.RemoveAudienceByFilter:
					return BulkEmailQueueMessage.Create<RemoveAudienceByFilterQueueMessage>(id, json, retries);
				case BulkEmailQueueMessageType.RemoveAllAudience:
					return BulkEmailQueueMessage.Create<RemoveAllAudienceQueueMessage>(id, json, retries);
				case BulkEmailQueueMessageType.SendTriggerEmail:
					return BulkEmailQueueMessage.Create<SendTriggerEmailQueueMessage>(id, json, retries);
				default:
					return baseMessage;
			}
		}
