namespace Terrasoft.Configuration.ReportService
{
	...
	public class ReportService : BaseService
	{
		...
		private GridUtilitiesServiceHandler GetGridUtilitiesServiceHandler() {
			return ClassFactory.Get<GridUtilitiesServiceHandler>();
		}

		private string[] GetRecordsIdFromFilters(string entitySchemaName, string filtersConfig) {
			GridUtilitiesServiceHandler utilities = GetGridUtilitiesServiceHandler();
			return utilities.GetPrimaryColumnValuesFromFilters(entitySchemaName, filtersConfig);
		}

		private string GetEntitySchemaNameByTemplateId(Guid templateId) {
			var entitySchemaManager = UserConnection.EntitySchemaManager;
			var entitySchema = entitySchemaManager.GetInstanceByName("SysModuleReport");
			var esq = new EntitySchemaQuery(entitySchema) {
				UseAdminRights = false
			};
			var entityNameColumn = esq.AddColumn("SysEntitySchema.Name");
			var moduleEntityNameColumn = esq.AddColumn("SysModule.SysModuleEntity.[SysSchema:UId:SysEntitySchemaUId].Name");
			var record = esq.GetEntity(UserConnection, templateId);
			var entitySchemaName = record.GetTypedColumnValue<string>(entityNameColumn.Name);
			return entitySchemaName.IsNullOrEmpty()
				? record.GetTypedColumnValue<string>(moduleEntityNameColumn.Name)
				: entitySchemaName;
		}
		...
		[WebInvoke(Method = "POST", RequestFormat = WebMessageFormat.Json, ResponseFormat = WebMessageFormat.Json,
			BodyStyle = WebMessageBodyStyle.Wrapped)]
		public string[] CreateReportsListByFilter(string templateId,
			string filtersConfig, bool convertInPDF) {
			var entitySchemaName = GetEntitySchemaNameByTemplateId(new Guid(templateId));
			var recordIds = GetRecordsIdFromFilters(entitySchemaName, filtersConfig);
			return CreateReportsList(string.Empty, string.Empty, templateId, recordIds, string.Empty, convertInPDF);
		}
	...
	public class AdditionalMacrosUtility : BaseOpenXmlUtility
	{
		...
		private bool GetCanUseFileFactory(Entity entity, EntitySchemaColumn entityColumn) {
			EntitySchemaManager entitySchemaManager = userConnection.EntitySchemaManager;
			ISchemaManagerItem<EntitySchema> entityManagerItem = entitySchemaManager.FindItemByName(entity.SchemaName);
			ISchemaManagerItem<EntitySchema> fileManagerItem = entitySchemaManager.FindItemByName("File");
			bool isFileInheritance = entitySchemaManager.IsInheritedFrom(entityManagerItem.UId,
				fileManagerItem.UId);
			if (!isFileInheritance) {
				return false;
			}
			EntitySchemaColumn dataColumn = fileManagerItem.Instance.Columns.GetByName("Data");
			bool isSameFileDataColumn = dataColumn.UId == entityColumn.UId;
			return isSameFileDataColumn;
		}
		...
		private byte[] GetReportBinaryColumnValue(Entity entity, EntitySchemaColumn entityColumn) {
			...
			bool canUseFileFactory = GetCanUseFileFactory(entity, entityColumn);
			if (canUseFileFactory) {
				try {
					var fileLocator = new EntityFileLocator(entity.SchemaName, id);
					IFile file = FileFactory.Get(fileLocator);	
					return GetFileValue(file);
				} catch (Exception exception) {
					_log.ErrorFormat("Report. Can't load file from entity: {0}, record: {1}. " +
						"ErrorMessage: {2}" +
						"StackTrace: {3}", 
						entity.SchemaName, id, exception.Message, exception.StackTrace);
				}
			}
