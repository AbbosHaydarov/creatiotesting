define("HomePageFunnelDesigner",
	 ["HomePageFunnelDesignerResources", ...(Terrasoft.UseMfForAngularElements
		 ? ["mf!SchemaViewComponent", "css-ltr!SchemaViewComponentStyles"]
		 : ["SchemaViewComponentOld"]), "FunnelWidgetComponent", "ExtWidgetConfigConverter",
	...
	return {
		...
		methods: {
			...
			initChartDesigner: function() {
				...
				this.set("dataSourceConfig", widgetConfig.dataSourceConfig);
				if (config) {
					const { title } = config;
